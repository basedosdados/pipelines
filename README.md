...
Onde `datasets` é o nome da agência onde o projeto será alocado. 

Isso irá criar um novo diretório com o nome `nome-do-projeto` em `pipelines/datasets` com o template padrão, já adaptado ao nome do projeto. O nome do projeto deve estar em [snake case](https://en.wikipedia.org/wiki/Snake_case) e deve ser único. Qualquer conflito com um projeto já existente será reportado.

O comando anterior assume que `datasets` já existe. Para criar uma nova agência, basta fazer:

```
python manage.py add-agency nome-agencia
```
