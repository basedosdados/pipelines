...
def load_flows_from_file(file_path: str) -> dict[str, Flow]:
    ...
    try:
        ...
        print(f"  Pulando {file_path}: ImportError (provavelmente Prefect 0.x) — {e}")
        return {}
    ...
    for name, obj in vars(module).items():
        if isinstance(obj, Flow):
            if obj.fn.__code__.co_filename == str(path.resolve()):
                flows[name] = obj
...
def deploy_flow(
    ...
    try:
        ...
        status = "(sem schedule)" if not schedules else f"com schedules: {schedules}"
...
def main():
    ...
    parser.add_argument("--branch", required=True, help="Branch do repositório")
    parser.add_argument("--files", nargs="*", help="Arquivos específicos para deploy")
    parser.add_argument("--all", action="store_true", help="Deploy de todos os flows")
