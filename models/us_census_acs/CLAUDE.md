...
- **Dictionary descriptions wrap across physical lines.** Reading only the first line truncates mid-sentence (`racnhpi` → "…(Native", `powsp05` → "…State or foreign country"). `parse_pums_dict.py` accumulates continuation lines until a value-label line, a `NOTE:`, or the next variable.
- **A wrapped description often continues onto a bare year** (`…prior to` / `2012`), which a naive page-number filter eats — reintroducing the truncation. These dicts run ~150 pages, so a standalone `19xx`/`20xx` is always a year, never a page number (`TXT_YEAR_RE`).
- **Some dictionaries are double-encoded UTF-8** (correct bytes decoded as latin-1 then re-encoded), so an en dash arrives as `â\x80\x93`. `clean_desc()` round-trips `.encode("latin-1").decode("utf-8")` and falls back when that fails (genuinely latin-1 files). It also collapses the tabs/newlines that wrapping introduces and strips the trailing period.
- **Audit descriptions after any parser change**, not just the columns you expected to fix: scan all of them for a dangling final word (`to`, `for`, `based`, `SOC`…), mojibake, tabs and trailing periods. That sweep is what caught `migpuma00` still truncated after the first fix looked green.
