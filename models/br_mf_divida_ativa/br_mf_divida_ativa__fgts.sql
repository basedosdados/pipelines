-- Fonte: PGFN Dados Abertos da Divida Ativa da Uniao (FGTS)
{{
    config(
...
select
    ...
    safe_cast(entidade_responsavel as string) entidade_responsavel,
    safe_cast(unidade_inscricao as string) unidade_inscricao,
...
from {{ set_datalake_project("br_mf_divida_ativa_staging.fgts") }} as t
