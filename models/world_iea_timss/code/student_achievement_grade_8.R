# Desempenho do aluno #bsa ----------------------------------------------
library(dplyr)    
library(readxl)  
library(haven) 

# ------------------------------------------------------------------------------
# Desempenho do aluno (Grade 8) - Processamento dos dados TIMSS 2023
# ------------------------------------------------------------------------------

# Define diretórios de entrada e saída
input_dir <- "~/BD/timms/TIMSS2023_IDB_R_G8/TIMSS2023_IDB_R_G8/2_Data Files/R Data"
output_dir <- "~/BD/timms/timms_2023/tabelas/desempnho_aluno_g8"
...
#Lista todos os arquivos .RData de desempenho do aluno (que começam com 'bsa')
arquivos <- list.files(
  path = input_dir,
  pattern = "^bsa.*\\.RData$",  # Expressão regular para arquivos .RData iniciando com 'bsa'
  ignore.case = TRUE,
  full.names = TRUE
)

# Lê a tabela de arquitetura com os nomes originais e novos das variáveis
arquitetura_desempenho_aluno <- readxl::read_excel(
  "~/BD/timms/timms_2023/arquitetura/[arquitetura]world_iae_timss_student_achievement_grade_8.xlsx")

# Define colunas de interesse com base na arquitetura
colunas <- arquitetura_desempenho_aluno$original_name
...
world_iae_timss_student_achievement_grade_8 <- do.call(rbind, lista_dados)
...
nomes_antigos <- arquitetura_desempenho_aluno$original_name
nomes_novos   <- arquitetura_desempenho_aluno$name

# Filtra apenas os nomes existentes no dataframe
nomes_validos <- nomes_antigos[nomes_antigos %in% colnames(world_iae_timss_student_achievement_grade_8)]
nomes_correspondentes <- nomes_novos[nomes_antigos %in% colnames(world_iae_timss_student_achievement_grade_8)]
...
ordem_desejada <- arquitetura_desempenho_aluno$name
ordem_final <- ordem_desejada[ordem_desejada %in% colnames(world_iae_timss_student_achievement_grade_8)]
...
df_final <- arquitetura_desempenho_aluno %>%
...
for (i in seq_len(nrow(df_final))) {
  ...
  if (!is.na(missing_val) && col_name %in% names(world_iae_timss_student_achievement_grade_8)) {
    world_iae_timss_student_achievement_grade_8[[col_name]] <- ifelse(
      world_iae_timss_student_achievement_grade_8[[col_name]] == missing_val,
      NA,
      world_iae_timss_student_achievement_grade_8[[col_name]]
    )
  }
}
