...
UNIT = col(
    "unit",
    "STRING",
    "Unidade de medida da série",
    "Unit of measure of the series",
    "Unidad de medida de la serie",
    obs="Rótulo BEA CL_UNIT.",
    original="CL_UNIT",
)
UNITM = col(
    "unit_mult",
    "STRING",
    "Multiplicador de base 10 aplicado ao valor",
    "Base-10 multiplier applied to the value",
    "Multiplicador de base 10 aplicado al valor",
    obs="Expoente; por exemplo, 6 = milhões, 9 = bilhões.",
    original="UNIT_MULT",
)
NOTEREF = col(
    "note_ref",
    "STRING",
    "Referência de nota de rodapé da BEA para a observação",
    "BEA footnote reference for the observation",
    "Referencia de nota al pie de la BEA para la observación",
    original="NoteRef",
)
GEONAME = col(
    "geo_name",
...
def table_id(example):
    return col(
        "table_id",
        "STRING",
        "Identificador da tabela BEA",
        "BEA table identifier",
        "Identificador de la tabla BEA",
        dic="yes",
        obs=f"Por exemplo, {example}.",
        original="TableName",
    )
...
TABLES["nipa"] = [
    ...
    col(
        "series_id",
        "STRING",
        "Código único da série NIPA",
        "Unique NIPA series code",
        "Código único de serie NIPA",
        obs="Por exemplo, A191RC.",
        original="SeriesCode",
    ),
    LINEDESC,
    col(
...
TABLES["gdp_by_industry"] = [
    ...
    col(
        ...
        "STRING",
        "Descrição da indústria ou do componente",
        "Description of the industry or component",
        "Descripción de la industria o del componente",
        obs="Nas tabelas de composição (por exemplo, TableID 6, 7, 25, 26) identifica o "
        "componente do agregado, não a indústria.",
...
TABLES["regional_state"] = [
    ...
    FREQ,
    geo_fips(
        "Código geográfico BEA da entidade estadual",
    ...
    col(
        ...
        obs="Dois dígitos; nulo para os EUA e as regiões BEA.",
        original="GeoFips",
    ),
    GEONAME,
    table_id("SAGDP2"),
...
TABLES["regional_metro"] = [
    YEAR,
    geo_fips(
        "Código CBSA da área metropolitana",
        "CBSA code of the metropolitan area",
        "Código CBSA del área metropolitana",
        "Cinco dígitos.",
    ),
    col(
        "id_cbsa",
