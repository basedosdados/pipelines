{{
    config(
        schema="au_doe_higher_education",
        alias="student_equity_performance",
        materialized="table",
        partition_by={
            "field": "year",
            "data_type": "int64",
            "range": {"start": 2011, "end": 2029, "interval": 1},
        },
    )
}}


select
    safe_cast(year as int64) year,
    safe_cast(institution_id as string) institution_id,
    safe_cast(state_abbreviation as string) state_abbreviation,
    safe_cast(student_group as string) student_group,
    safe_cast(equity_group as string) equity_group,
    safe_cast(address_basis as string) address_basis,
    safe_cast(equity_group_label as string) equity_group_label,
    safe_cast(commencing_students as int64) commencing_students,
    safe_cast(enrolments as int64) enrolments,
    safe_cast(award_course_completions as int64) award_course_completions,
    safe_cast(access_rate as float64) access_rate,
    safe_cast(participation_rate as float64) participation_rate,
    safe_cast(participation_ratio as float64) participation_ratio,
    safe_cast(retention_rate as float64) retention_rate,
    safe_cast(retention_ratio as float64) retention_ratio,
    safe_cast(success_rate as float64) success_rate,
    safe_cast(success_ratio as float64) success_ratio,
    safe_cast(attainment_rate as float64) attainment_rate
from
    {{
        set_datalake_project(
            "au_doe_higher_education_staging.student_equity_performance"
        )
    }} as t
