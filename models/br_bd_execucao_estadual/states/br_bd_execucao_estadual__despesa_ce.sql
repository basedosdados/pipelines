...
-- exports, 2006-2026. See code/download_ce.py and code/clean_ce.py; the source's
-- defects
-- are recorded at length in models/br_bd_execucao_estadual/SOURCE_LESSONS.md (CE
-- section).
--
-- **CE publishes empenho at ONE ROW PER EMPENHO, not per budget line.** Measured on
-- 2025:
-- 196,255 rows over 30,600 distinct `numero`, but `(unidade_gestora, numero)` is
-- unique --
-- the número simply restarts per unidade gestora, the SC pattern. So the empenho key is
-- (exercicio, unidade gestora, numero) and no pivot across budget lines is needed.
-- About
-- 1% of keys repeat across the whole series (27k of 4.93M, mostly same natureza) -- the
-- same near-duplicate publishing MG shows at 9.3%; `despesa` carries no uniqueness
-- test for
-- exactly this reason, so every row is kept.
--
-- **THREE naming eras are stacked in one staging table** (constants.CE_SCHEMAS): legacy
-- 2006-2013 (`num_ano`, `cod_*`, `vlr_*`), mid 2014-2018 (`exercicio`, `valor*`,
-- names AND
-- codes) and the portal eras 2018+ (`unidade_gestora`, `valor_*`, labels only). Each
-- row
-- populates exactly one era's columns, so every field below is a COALESCE across the
-- eras'
-- columns; the value that is present is the row's own. `_final` columns already carry
-- the
-- net of suplementação and anulação, verified: valor_empenhado_final =
-- valor_empenhado +
-- valor_suplementado - valor_anulado to the cent.
--
-- **valor_empenhado and valor_pago are NET** (empenhado + suplementado - anulado;
-- pago -
...
--
-- **valor_liquidado is NULL for CE, and that is measured, not an omission.** CE
-- publishes a
-- liquidação document set (ce_liquidacao), but it does NOT link to the empenho: its
-- `unidade_gestora` code system overlaps the empenho's on only 120 of 982 codes, and
-- the
-- empenho `numero` fans out ~93x on its own, so no (exercicio, gestora, numero) join
-- resolves (23.6% match, and fanned out where it does). Attributing a liquidação
-- value to
-- an empenho would be invention. CE's liquidação lives in the `liquidacao` table
-- instead,
-- where each document stands on its own (with `id_empenho_bd` null, the PB-pagamento
-- precedent). The empenho row itself carries no `valorliquidado` -- the column exists
-- in the
-- mid-era schema but is empty on all 4.93M rows.
--
-- **Decimal format varies by era and sub-source but never carries a thousands
-- separator**
-- (measured: no value contains both '.' and ','), so `replace(x, ',', '.')` parses
-- every
-- era -- dot, comma and integer alike -- without an SP-style thousands strip.
--
-- **The code dimensions (categoria..item, funcao..acao, fonte) are era-limited.**
-- Only the
-- legacy and mid eras publish a natureza CODE (`cod_item_natureza` / `natureza`); the
-- portal
-- eras publish only LABELS (`natureza_da_despesa`, `funcao`, ...), which cannot be
-- split
-- into the 8-digit natureza code, so the code-split columns are null for 2019+. The
-- função/subfunção/programa/ação and fonte columns are populated only by the portal
-- eras,
-- which publish them as labels -- CE issues no separate codes for them -- so those
-- columns
-- carry CE's label where present and are null otherwise. This mirrors the reality
-- that each
-- state's dimension columns are in that state's own scheme and are not cross-state
-- codes.
with
    fonte as (
        select *
    ...
    base as (
        select
            ...
            coalesce(nullif(trim(numero), ''), nullif(trim(cod_ne), '')) as num,
            coalesce(
            ...
            -- gestora name where the era gives one (portal); legacy/mid have only a
            -- code.
            nullif(trim(unidade_executora), '') as nome_ug,
            -- Net empenhado. Portal: valor_empenhado_final. Legacy/mid: empenhado +
            -- suplementado - anulado. Each era's other terms are null, so coalesce
            -- picks
            -- the row's own era.
            coalesce(
                ...
                safe_cast(replace(valor, ',', '.') as float64) + coalesce(
                    safe_cast(replace(valorsuplementado, ',', '.') as float64), 0
                )
                - coalesce(safe_cast(replace(valoranulado, ',', '.') as float64), 0)
            ) as valor_empenhado,
            -- Net pago. Portal: valor_pago_final (2019 Q4 reduzido has only
            -- valor_pago).
            -- Legacy/mid: pago - anulado_pago.
            coalesce(
                safe_cast(replace(valor_pago_final, ',', '.') as float64),
                safe_cast(replace(valor_pago, ',', '.') as float64),
                safe_cast(replace(vlr_pago, ',', '.') as float64) - coalesce(
                    safe_cast(replace(vlr_anulado_pago, ',', '.') as float64), 0
                ),
            ...
            -- description: legacy dsc_linha, mid especificacaogeral. Portal publishes
            -- none.
            coalesce(
                nullif(trim(especificacaogeral), ''), nullif(trim(dsc_linha), '')
            ) as descricao,
            -- tender modality label where present (legacy code, mid label). No tender
            -- table.
            coalesce(
            ...
            -- partial document prefix inside the name ("54.212.382 FELLIPE ..."), so
            -- it is
            ...
            -- natureza CODE: legacy only (8-10 digits, C G MM EE + item). Mid's
            -- `natureza`
            -- is the movement type (Ordinária/Anulação/Suplementação), not a code,
            -- and its
            -- `classiforcamreduz` is only a reduced code; the portal eras give a
            -- label. So
            ...
            coalesce(nullif(trim(programa_de_governo), '')) as programa,
            coalesce(nullif(trim(acao_de_governo), '')) as acao,
        ...
        where
            ...
            -- Keep only ORIGINAL empenho documents. Legacy (cod_movimento 11,
            -- 1,792,704 of
            -- 1,859,177) and mid (natureza Ordinária, 1,122,129 of 1,177,759) each
            -- publish
            -- their anulações and suplementações BOTH as separate documents with
            -- their own
            -- número AND as columns on the original row. Keeping the documents
            -- double-counts
            -- (measured: mid Anulação docs R$4,455.7M = Ordinária valoranulado
            -- R$4,451.0M;
            -- Suplementação docs R$165.8M = valorsuplementado R$165.8M, exact) and
            -- invents
            -- phantom empenhos, so they are dropped and the net is taken from the
            -- original
            ...
            and not (
                ...
                and coalesce(nullif(trim(natureza), ''), '')
                in ('Anulação', 'Suplementação')
            )
    )
select
    ...
    -- document: mid CNPJ/CPF column, legacy internal credor code, or the numeric
    -- prefix the
    ...
    case
        when
            doc_credor_mid like '%/%'
            or regexp_contains(credor_raw, r'^[0-9]{2}\.[0-9]{3}\.[0-9]{3}')
        then 'PJ'
        when
            doc_credor_mid like '%*%' or regexp_contains(doc_credor_mid, r'^[0-9]{11}$')
        then 'PF'
    ...
    -- Legacy natureza is 8-10 digits (C G MM EE SS [+ item]); split the standard
    -- first 8.
    case
        when regexp_contains(cod_natureza, r'^[0-9]{8,10}$')
        then substr(cod_natureza, 1, 1)
    end as categoria_economica,
    case
        when regexp_contains(cod_natureza, r'^[0-9]{8,10}$')
        then substr(cod_natureza, 2, 1)
    end as grupo_despesa,
    case
        when regexp_contains(cod_natureza, r'^[0-9]{8,10}$')
        then substr(cod_natureza, 3, 2)
    end as modalidade_aplicacao,
    case
        when regexp_contains(cod_natureza, r'^[0-9]{8,10}$')
        then substr(cod_natureza, 5, 2)
    end as elemento_despesa,
    case
        when regexp_contains(cod_natureza, r'^[0-9]{8,10}$')
        then substr(cod_natureza, 7, 2)
    end as item_despesa,
    ...
    -- Measured: CE's liquidação cannot be linked to the empenho (see header); the
    -- liquidação
