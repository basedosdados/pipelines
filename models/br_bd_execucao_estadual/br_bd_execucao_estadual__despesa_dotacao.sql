...
-- execução ao fim de cada exercício, por linha orçamentária completa -- não um
-- movimento,
...
-- (dadosabertos.rj.gov.br, pacote tfe-despesa). Sem credor, sem empenho, sem data
-- abaixo
-- do mês: é o retrato da dotação, com dotação inicial e atual, empenhado, liquidado,
-- pago,
-- restos a pagar e o desembolso total. Valores em reais correntes, com vírgula
-- decimal na
-- origem.
...
select
    ...
    -- Reais correntes, vírgula decimal na origem (às vezes sem casas): remove o ponto
    -- de
    -- milhar e troca a vírgula por ponto.
    safe_cast(
        replace(replace(valor_dotacao_inicial, '.', ''), ',', '.') as float64
    ) as valor_dotacao_inicial,
    safe_cast(
        replace(replace(valor_dotado_atual, '.', ''), ',', '.') as float64
    ) as valor_dotacao_atual,
    safe_cast(
        replace(replace(valor_despesa_autorizada, '.', ''), ',', '.') as float64
    ) as valor_despesa_autorizada,
    safe_cast(
        replace(replace(valor_empenhado, '.', ''), ',', '.') as float64
    ) as valor_empenhado,
    safe_cast(
        replace(replace(valor_liquidado, '.', ''), ',', '.') as float64
    ) as valor_liquidado,
    safe_cast(replace(replace(valor_pago, '.', ''), ',', '.') as float64) as valor_pago,
    safe_cast(
        replace(replace(valor_rp_a_pagar, '.', ''), ',', '.') as float64
    ) as valor_restos_a_pagar,
    safe_cast(
        replace(replace(valor_rp_pago, '.', ''), ',', '.') as float64
    ) as valor_restos_a_pagar_pago,
    safe_cast(
        replace(replace(valor_total_desembolsado, '.', ''), ',', '.') as float64
    ) as valor_total_desembolsado
from origem
