...
def clean_parliamentary_elections():
    ...
    arch_df = pd.read_excel('code/architecture_table_parliamentary_elections.xlsx')
    ...
    excluded_cols = arch_df[arch_df['name'] == '(excluded)']['original_name'].tolist()
    ...
    if 'date' in df.columns:
        ...
        df['date'] = df['date'].replace('', None)  # Empty strings to None for DATE type
    ...
    flag_cols = [
        'flag_constituent', 'flag_inconsequential', 'flag_coup',
        'flag_vacant_seats', 'flag_appointed', 'flag_non_partisan'
    ]
...
def clean_presidential_elections():
    ...
    # Load architecture table to get column mappings
    arch_df = pd.read_excel('code/architecture_table_presidential_elections.xlsx')
    ...
    flag_cols = [
        'flag_two_round', 'flag_inconsequential', 'flag_coup',
        'flag_plebiscite', 'flag_unopposed', 'flag_indirect'
    ]
    for col in flag_cols:
        if col in df.columns:
            df[col] = df[col].astype('boolean')
...
if __name__ == '__main__':
    import os
    os.chdir('/Users/rdahis/Monash Uni Enterprise Dropbox/Ricardo Dahis/Academic/Data/World/National Elections Database')
