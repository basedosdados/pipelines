...
def build(path_dados, path_queries, comp, api_dir, first_year, last_year):
    ...
    for ano in range(first_year, last_year + 1):
        ...
        if not rows:
            print(
                f"  municipio_despesas_funcao {ano}: no data in API files, skipping"
            )
            continue
        ...
        print(
            f"  municipio_despesas_funcao {ano}: {len(df_all):,} rows from {len(json_files)} municipalities"
        )
        partition_and_save(
            df_all, "municipio_despesas_funcao", ano, path_dados
        )
