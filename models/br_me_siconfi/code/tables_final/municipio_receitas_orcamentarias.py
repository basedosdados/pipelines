"""municipio_receitas_orcamentarias builder.

Sources:
  1989-2012: input/municipio/quadro{ano}_{1-3}.xlsx  (Excel wide format)
...
def _build_legacy(path_dados, comp, ano):
    ...
    arquivos = [
        ...
        if int(arq[-11:-7]) == ano
        and (
            (1989 <= ano <= 1996 and int(arq[-6:-5]) <= 3)
            or (1997 <= ano <= 2012 and int(arq[-6:-5]) == 1)
        )
    ]

    if not arquivos:
        print(
            f"  municipio_receitas_orcamentarias {ano}: no legacy files found, skipping"
        )
        return pd.DataFrame()
    ...
    for arquivo in arquivos:
        ...
        df_dados = df_dados.merge(
            df_comp_receitas, how="left", on=["ano", "conta"]
        )
    ...
    print(
        f"  municipio_receitas_orcamentarias {ano}: {len(df):,} rows (legacy)"
    )
