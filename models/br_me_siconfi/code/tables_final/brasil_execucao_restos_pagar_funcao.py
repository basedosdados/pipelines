...
def build(path_dados, path_queries, comp, year_data, ano):
    ...
    if df is None or df.empty:
        print(
            f"  brasil_execucao_restos_pagar_funcao {ano}: no data, skipping"
        )
        return pd.DataFrame()
    ...
    df = df[ORDEM_BRASIL_SIMPLES]
    df["valor"] = pd.to_numeric(df["valor"], errors="coerce").astype("float")

    print(f"  brasil_execucao_restos_pagar_funcao {ano}: {len(df):,} rows")
    partition_and_save_brasil(
        df, "brasil_execucao_restos_pagar_funcao", ano, path_dados
    )
    return pd.DataFrame()
