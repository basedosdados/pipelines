...
def build(path_dados, path_queries, comp, year_data, ano):
    ...
    if df is None or df.empty:
        print(f"  uf_receitas_orcamentarias {ano}: no data, skipping")
        return pd.DataFrame()
    ...
    n = df["id_uf"].nunique()
    print(
        f"  uf_receitas_orcamentarias {ano}: {len(df):,} rows from {n} states"
    )
    partition_and_save(df, "uf_receitas_orcamentarias", ano, path_dados)
    return unmatched
