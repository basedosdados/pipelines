...
def build(path_dados, path_queries, comp, year_data, ano):
    ...
    if df is None or df.empty:
        print(f"  municipio_execucao_restos_pagar {ano}: no data, skipping")
        return pd.DataFrame()
    ...
    print(
        f"  municipio_execucao_restos_pagar {ano}: {len(df):,} rows from {n} municipalities"
    )
    partition_and_save(df, "municipio_execucao_restos_pagar", ano, path_dados)
    return pd.DataFrame()
