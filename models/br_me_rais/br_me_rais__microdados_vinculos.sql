...
{% macro vinculos_select(source_table) %}
    select
        ...
        safe_cast(
            indicador_portador_deficiencia as string
        ...
        trim(
            safe_cast(regexp_replace(distritos_sp, r'^0+', '') as string)
        ) as distritos_sp,
        trim(
    ...
    from {{ set_datalake_project(source_table) }}
{% endmacro %}

with
    pre_2023 as ({{ vinculos_select("br_me_rais_staging.microdados_vinculos") }}),
    from_2023 as ({{ vinculos_select("br_me_rais_staging.microdados_vinculos_2023") }})
