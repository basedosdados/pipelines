...
annual. Status: **architecture design, pre-upload.**
...
3. **Full history + per-vintage NAICS directories.** Keep all NAICS-era years
   (1998–2023). `naics` is one STRING column FK→`naics_2022` (canonical/lineage);
   referential integrity enforced per-vintage in dbt via `naics_version`.
...
| CBP years | NAICS vintage | `naics_version` | unresolved vs naics_2022 |
|---|---|---|---|
| 2022–2023 | 2022 | `2022` | 0% |
| 2017–2021 | 2017 | `2017` | 0% |
| 2012–2016 | 2012 | `2012` | 2.1% (43 codes) |
| 2008–2011 | 2007 | `2007` | 11.5% (246 codes) |
| 2003–2007 | 2002 | `2002` | larger (not yet measured) |
| 1998–2002 | 1997 | `1997` | larger |
...
`44/45`, `48/49` stay native (directory stores range `31-33` etc.) → 7 sector codes
excluded from the naics test (documented).
...
`id_county`(county), `naics`(STRING, FK `naics_2022:id_naics` †), `naics_version`
...
- † `naics` FK is canonical→`naics_2022`; per-vintage integrity via dbt (see below).
...
`models/br_bd_diretorios_us/code/clean.py:868`). Against the existing `naics_2022`
(official convention, stores sector ranges `31-33`), the 7 sector split-codes
31/32/33/44/45/48/49 are the only test exception, and only for 2017–2023.
...
and restructured construction 233/234/235). 1998–2002 data resolves at **0.09%**
(only admin codes `95`,`99`, ignored). So there are **four** vintage dirs, not three:
`naics_1997/2002/2007/2012`; the FK mapping is `1997→naics_1997`, `2002→naics_2002`,
`2007→naics_2007`, `2012→naics_2012`, `2017/2022→naics_2022`.
...
- `dbt_utils.unique_combination_of_columns` on the logical key (national/state).
  **county** uses the relaxed `custom_unique_combinations_of_columns`
  (`proportion_allowed_failures: 0.001`) — the 1999 source file repeats a block for
  FIPS 01045 (Dale County, AL); 5 pairs are byte-identical (dropped by the cleaner's
  dedupe) and NAICS 62111 has one genuinely conflicting pair, kept faithfully.
- `not_null_proportion_multiple_columns` at 0.05 with `ignore_values` for the
  per-band and flag columns (legitimately empty for most small cells / eras).
...
5. **[sub-project]** build + upload 3 naics vintage directories (2002/2007/2012)
6. upload CBP parquet → BigQuery dev/staging
7. dbt models + schema.yml (+ vintage tests)
8. validate
9. discover ids → metadata (dev/staging)
10. [PAUSE checkpoint]
11. metadata prod → PR
...
- **NAICS 1997 source** — see Prerequisite; default FK 1998–2002→naics_2002.
