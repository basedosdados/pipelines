...
def map_to_id_municipio_7(
    series: pd.Series, directory_df: pd.DataFrame
) -> pd.Series:
    cleaned = standardize_id(series)
    id_map = {}
    if (
        "id_municipio_6" in directory_df.columns
        and "id_municipio" in directory_df.columns
    ):
        id_map = dict(
            zip(
    ...
    for sheet in sheet_names:
        ...
        if master_df is None:
            ...
            new_cols = [
                c
                for c in df.columns
                if c not in master_df.columns or c == "__id_municipio__"
            ]
            df = df[new_cols]
...
def build_section_year(
    ...
    if "__id_municipio__" in master_df:
        id_series = map_to_id_municipio_7(
            master_df["__id_municipio__"], directory_df
        )
    else:
    ...
    if (
        "sigla_uf" in directory_df.columns
        and "id_municipio" in directory_df.columns
    ):
    ...
    for _, row in arch_df.iterrows():
        ...
        if (
            clean_name is None
            or pd.isna(clean_name)
            or str(clean_name).strip() == ""
        ):
            continue
            ...
            if (
                ...
                if isinstance(original, str):
                    fallback_candidates.append(
                        original.replace("_ano_lei", "_ano")
                    )
                for candidate in fallback_candidates:
                    col_name = find_column_name(
                        candidate, columns, lower_map, norm_map
                    )
                    if col_name:
                        break
            if col_name:
...
def section_exists_in_year(
    mapping_df: pd.DataFrame, section: str, year: int
) -> bool:
    if mapping_df.empty:
        return True
    year_col = str(year)
...
def run(sections: list[str]) -> None:
    ...
    for section in sections:
        ...
        for year in years:
            year_col = f"original_name_{year}"
            if (
                year_col not in arch_df.columns
                or not arch_df[year_col].notna().any()
            ):
                continue
            ...
            year_df = build_section_year(
                section, year, arch_df, master_cache[year], directory_df
            )
        ...
        if "id_municipio" in final_df.columns:
            final_df = final_df[
                final_df["id_municipio"].notna()
                & (final_df["id_municipio"].astype(str).str.strip() != "")
            ]
        ...
        if len(string_cols) > 0:
            final_df[string_cols] = final_df[string_cols].replace(
                text_replacements
            )
        out_path = OUTPUT_DIR / f"{section}.csv"
        final_df.to_csv(out_path, index=False)
        print(
            f"  Wrote {out_path.relative_to(BASE_DIR)} ({len(final_df)} rows)"
        )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Clean MUNIC data into section CSVs."
    )
    parser.add_argument(
        "--sections",
        type=str,
