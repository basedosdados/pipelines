...
**Harvested so far** (`~/Downloads/br_mgi_pncp_data/input`, 441 MB):
...
- `pipelines/datasets/br_mgi_pncp/` — the canonical home for the pure transform:
  ...
  inline schedule).
- `models/br_mgi_pncp/code/` — one-shot onboarding front ends over that package:
  `download.py`, `clean.py`, `build_dicionario.py`, `upload.py`, plus
  `architecture/build_architecture.py` and `gen_dbt.py`.
- `models/br_mgi_pncp/` — dbt models and `schema.yml`, both generated by `code/gen_dbt.py`
...
`pipelines/datasets/br_mgi_pncp/tests/` covers the transform's silent-failure modes — NULL
...
The dataset id `br_mgi_pncp` has no organization segment. That is deliberate and
...
`build_dicionario_task`, `models/br_mgi_pncp/code/build_dicionario.py` and the
...
re-harvested under stamped names. `br_mgi_pncp_staging.dicionario` (external) was
...
`staging/br_mgi_pncp/dicionario/` is empty and there is no staging table — the
...
deliberately switches to `dump_mode="overwrite"`. br_mgi_pncp was exposed because
...
frontend until the PR merges, table-approve materialises `basedosdados.br_mgi_pncp.*`
...
list is kept at `~/Downloads/br_mgi_pncp_data/deleted_empty_chunks.json`.
...
| staging table | rows |
|---|---|
| `br_mgi_pncp_staging.contrato` | 4,707,847 |
| `br_mgi_pncp_staging.ata_registro_preco` | 1,137,524 |
| `br_mgi_pncp_staging.instrumento_cobranca` | 215,382 |
...
The single error is `Not found: Table basedosdados-dev:br_mgi_pncp.contratacao` --
...
- Selecting `br_mgi_pncp__dicionario` pulls in **contratacao's** test too, since
