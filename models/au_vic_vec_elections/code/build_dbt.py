"""Generate the au_vic_vec_elections dbt models and schema.yml.

Both are derived from ``pipelines/datasets/au_vic_vec_elections/schema.py`` so the
...
Run:  PYTHONPATH=. ~/.venvs/bd-pipelines-vic/bin/python models/au_vic_vec_elections/code/build_dbt.py
...
# A directory FK written as ``<dataset>.<table>:<column>`` maps to the dbt model that
# builds that directory, plus the field the relationships test binds to.
#
# ``br_bd_diretorios_data_tempo.ano`` needs ``ano.ano`` rather than ``ano``: the model's
