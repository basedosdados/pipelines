{{
    config(
        schema="us_usda_nass",
        alias="survey_agricultural_district",
        materialized="table",
        partition_by={
            "field": "year",
            "data_type": "int64",
...
with
    src as (
        select
            ...
            safe_cast(agricultural_district_code as string) agricultural_district_code,
            safe_cast(agricultural_district_name as string) agricultural_district_name,
        ...
        from
            {{
                set_datalake_project(
                    "us_usda_nass_staging.survey_agricultural_district"
                )
            }} as t
    )
...
qualify
    row_number() over (
        partition by
            ...
            domain_category,
            reference_period
        order by value desc, short_description, state_name, agricultural_district_name
    )
    = 1
