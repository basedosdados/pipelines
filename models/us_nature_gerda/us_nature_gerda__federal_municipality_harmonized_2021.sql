{{
    config(
        ...
        alias="federal_municipality_harmonized_2021",
        materialized="table",
        partition_by={
            "field": "year",
            "data_type": "int64",
...
select
    ...
    safe_cast(municipality_name as string) municipality_name,
    ...
    safe_cast(flag_naive_turnout_above_1 as string) flag_naive_turnout_above_1,
    safe_cast(flag_unsuccessful_naive_merge as string) flag_unsuccessful_naive_merge,
    safe_cast(flag_total_votes_incongruent as string) flag_total_votes_incongruent,
    safe_cast(party as string) party,
    safe_cast(vote_share as float64) vote_share
from
    {{
        set_datalake_project(
            "us_nature_gerda_staging.federal_municipality_harmonized_2021"
        )
    }} as t
