...
ANOS = range(2022, 2024 + 1)

for temp_ano in ANOS:
    df_ovinos = pd.read_parquet(
        f"{Path.cwd()}/output/producao_pecuaria/ovinos_tosquiados/parquet/ano={temp_ano}/data.parquet"
    )
    df_vacas = pd.read_parquet(
        f"{Path.cwd()}/output/producao_pecuaria/vacas_ordenhadas/parquet/ano={temp_ano}/data.parquet"
    )
    ...
    temp_export_file_path = f"{Path.cwd()}/output/producao_pecuaria/parquet/ano={temp_ano}/data.parquet"
