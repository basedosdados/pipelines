...
if __name__ == "__main__":
    ANOS_TRANSFORMADOS = get_existing_years(
        f"{Path.cwd()}/output/efetivo_rebanhos/parquet"
    )
    ARQUIVOS_JSON = list(
        Path(f"{Path.cwd()}/output/efetivo_rebanhos/json/").glob("*.json")
    )
    JSON_FALTANTES = [
        arquivo
    ...
    for path in JSON_FALTANTES:
        ...
        print("Transformações finalizadas!")
        temp_export_file_path = f"{Path.cwd()}/output/efetivo_rebanhos/parquet/ano={temp_ano}/data.parquet"
        print(
