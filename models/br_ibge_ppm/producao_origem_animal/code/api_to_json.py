...
ANOS_BAIXADOS = [
    ...
    for f in glob.glob(
        f"{Path.cwd()}/output/producao_origem_animal/json/*.json"
    )
]
...
async def main(
    ...
    for year in years:
        ...
        async with aiohttp.ClientSession(
            ...
            os.makedirs(
                f"{Path.cwd()}/output/producao_origem_animal/json",
                exist_ok=True,
            )
            with open(
                f"{Path.cwd()}/output/producao_origem_animal/json/{year}.json",
                "a",
            ) as f:
