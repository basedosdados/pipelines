...
FIELDS = ["name", "bigquery_type", "description", "temporal_coverage",
...
DICTIONARY = {
    "case_status", "visa_class", "full_time_position", "wage_unit_of_pay",
    "prevailing_wage_unit_of_pay", "prevailing_wage_level",
    "prevailing_wage_source", "agent_representing_employer", "h1b_dependent",
    "willful_violator", "support_h1b", "withdrawn", "secondary_entity",
    "application_type", "refile", "schedule_a_sheepherder",
    "required_experience", "is_multiple_worksites",
    "type_of_employer_application", "type_of_employer", "h2a_labor_contractor",
    "nature_of_temporary_need", "emergency_filing", "cap_exempt",
    "meals_provided", "frequency_of_pay", "class_of_admission",
    "minimum_education", "education_level", "foreign_worker_education",
}

UNITS = {
    "year": "year",
    "wage_offered_from": "usd", "wage_offered_to": "usd",
    "wage_offered_from_annual": "usd", "wage_offered_to_annual": "usd",
    "prevailing_wage": "usd", "prevailing_wage_annual": "usd",
    "overtime_rate_from": "usd", "overtime_rate_to": "usd",
    "piece_rate_offer": "usd",
    "total_workers": "person", "workers_requested": "person",
    "workers_certified": "person", "worksite_workers": "person",
    "new_employment": "person", "continued_employment": "person",
    "change_previous_employment": "person", "new_concurrent_employment": "person",
    "change_employer": "person", "amended_petition": "person",
    "employer_num_employees": "person", "housing_total_occupancy": "person",
    "employer_year_commenced_business": "year",
    "required_experience_months": "month", "work_experience_months": "month",
    "anticipated_number_of_hours": "hour",
    "total_worksite_locations": "unit", "total_worksite_records": "unit",
}
...
OBSERVATIONS = {
    ...
    "employer_state": "USPS abbreviation. Not linked to the US state directory, which is keyed on the FIPS code; referential coverage is checked by a dbt test instead",
    "worksite_state": "USPS abbreviation. Not linked to the US state directory, which is keyed on the FIPS code; referential coverage is checked by a dbt test instead",
    "housing_state": "USPS abbreviation",
...
PROGRAM_OBS = {
    ("lca", "wage_unit_of_pay"): "Absent from the FY2010 source file, so every FY2010 annualised wage is null",
    ("lca", "case_number"): "From FY2020 the source publishes one file per quarter; a fiscal year is the union of its quarterly files",
    ("perm", "prevailing_wage"): "Not published from FY2025, when the redesigned ETA-9089 replaced the wage amount with a reference to the prevailing wage determination case number",
    ("h2b", "prevailing_wage"): "Not published from FY2016; only the determination case number survives in prevailing_wage_tracking_number",
}
...
def main() -> int:
    ...
    for program in ["lca", "perm", "h2a", "h2b"]:
        ...
        for name, btype in cm.columns(program):
            ...
            rows.append({
                "name": name, "bigquery_type": btype, "description": en,
                "temporal_coverage": "", "covered_by_dictionary": covered,
            ...
            entry = {
                "name": name, "bigquery_type": btype,
                "description_pt": pt, "description_en": en, "description_es": es,
    ...
    with open(ARCH / "dictionary.csv", "w", newline="") as fh:
        ...
        for name, btype, en, _pt, _es in dict_cols:
            w.writerow({"name": name, "bigquery_type": btype, "description": en,
                        "temporal_coverage": "", "covered_by_dictionary": "no",
                        "directory_column": "", "measurement_unit": "",
                        "has_sensitive_data": "no", "observations": "",
                        "original_name": ""})
    (JSON_OUT / "dictionary.json").write_text(json.dumps(
        [{"name": n, "bigquery_type": t, "description_pt": pt,
          "description_en": en, "description_es": es,
          "covered_by_dictionary": False, "has_sensitive_data": False}
