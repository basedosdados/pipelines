...
DROP_RULES: list[tuple[str, str]] = [
    ...
    (r"^(EMPLOYER_POC|EMP_POC|EMP_CONTACT)_", "employer point-of-contact personal details"),
    ...
    (r"^(EMPLOYER_DECL_INFO|EMP_INFO_DECL|PREPARER_INFO)_", "declarant personal details"),
    (r"^(EMPLOYER|EMP|LAWFIRM_BUSINESS|ATTY_AG)_?(PHONE|PHONEEXT|FEIN)", "phone / employer identifier"),
    (r"^EMPLOYER_PHONE(_EXT)?$", "employer phone"),
    (r"^(PHONE|EMAIL|WEBSITE)_(TO_APPLY|EXT_TO_APPLY)$", "job application contact"),
    (r"^FOREIGN_WORKER_(INFO_)?(CITY|STATE)$|^FW_INFO_POSTAL_CODE$",
     "foreign worker residence — personal"),
    (r"^FOREIGN_WORKER_ED_INST", "foreign worker education institution address"),
    # Second and later worksite / wage blocks: only the primary worksite is published.
    (r"_(?:[2-9]|10)$|(?<=[A-Z])[2-9]$", "secondary worksite/wage block (primary worksite only)"),
    (r"^(PW_NON-OES_YEAR|PW_SURVEY_NAME|PW_SURVEY_PUBLISHER|PW_OTHER_SOURCE|"
     r"PW_SOURCE_NAME_OTHER)(_1)?$", "prevailing wage survey detail"),
    ...
    (r"^DOT_(OCCUPATIONAL_CODE|NAME)$", "legacy DOT occupation code (SOC is kept)"),
    ...
    (r"^(WAGE_RATE|RATE_PER|MAX_RATE|PART_TIME|CITY|STATE|PREVAILING_WAGE|WAGE_SOURCE|"
     r"YR_SOURCE_PUB|OTHER_WAGE_SOURCE)__?2$", "second worksite block"),
    # Recruitment / advertising process detail (PERM), not wage or employer facts.
    (r"^(RECR_INFO|RECR_OCC|RI_|REC_INFO|NOTICE_POST)", "PERM recruitment process detail"),
    (r"^(JOB_FAIR|ON_CAMPUS_RECRUITING|EMPLOYER_WEBSITE|PRO_ORG_AD|JOB_SEARCH_WEBSITE|"
     ...
     r"APP_FOR_COLLEGE|PROFESSIONAL_OCCUPATION)", "PERM recruitment process detail"),
    ...
    (r"^(STATE_OF_HIGHEST_COURT|NAME_OF_HIGHEST_STATE_COURT)$", "attorney bar detail"),
    (r"^(PUBLIC_DISCLOSURE|PUBLIC_DISCLOSURE_LOCATION|LABOR_CON_AGREE|"
     r"AGREE_TO_LC_STATEMENT|MASTERS_EXEMPTION)$", "LCA attestation plumbing"),
    ...
    (r"^(ON_CALL_REQUIREMENT|CERTIFICATION_REQUIREMENTS|DRIVER_REQUIREMENTS|"
     ...
     r"EMP_EXPERIENCE_REQD|OTHER_EDU|MAJOR$|SECOND_DIPLOMA)", "H-2 job-condition detail"),
    (r"^(HOUSING_ADDRESS_LOCATION|HOUSING_POSTAL_CODE|HOUSING_COUNTY|"
     ...
     r"MEAL_REIMBURSEMENT|TOTAL_ADDENDUM_A_RECORDS)", "H-2A housing detail"),
    (r"^(DEDUCTIONS_FROM_PAY|OTHER_FREQUENCY_OF_PAY|OVERTIME_PAY|"
     ...
     r"JOB_ORDER_TO_SWA|PREV_TRADE_NAME_DBA)", "H-2 contract / attestation detail"),
    (r"^(CAP_SUBJECT_WORKERS|CAP_EXEMPT_WORKERS)$", "H-2B cap breakdown"),
    (r"^(2nd|3rd)_PWD_CASE_NUMBER$", "additional prevailing-wage determination"),
    (r"^EMERGENCY_FILING_PWD", "emergency filing attachment"),
    (r"^OTHER_WORKSITE_LOCATION$", "free-text secondary worksite note"),
    (r"^PRIMARY_WORKSITE_(TYPE|BLS_AREA)$", "PERM worksite classification detail"),
    ...
    (r"^(CERTIFIED_BEGIN_DATE|CERTIFIED_END_DATE)$", "legacy certified period"),
    ...
    (r"^ALIEN_WORK_CITY$", "legacy H-2A worksite city, superseded by WORKSITE_CITY"),
    (r"^PART_TIME_1$", "legacy part-time flag, superseded by FULL_TIME_POSITION"),
]
...
def fiscal_year(key: str) -> int:
    return int(re.match(r"[a-z0-9]+_(\d{4})", key).group(1))
    ...
    for path in files:
        ...
        for col in cols:
            ...
            if canon and canon not in seen:
                ...
                per_program[prog].append(
                    {"fiscal_year": fy, "source_file": path.name,
                     "source_column": col, "canonical_column": canon,
                     "disposition": "mapped"}
                )
                continue
            if canon:  # duplicate alias hit — keep the first, record the rest
                per_program[prog].append(
                    {"fiscal_year": fy, "source_file": path.name,
                     "source_column": col, "canonical_column": "",
        ...
        for canon, _ in cm.columns(prog):
            ...
            if canon not in seen:
                per_program[prog].append(
                    {"fiscal_year": fy, "source_file": path.name,
                     "source_column": "", "canonical_column": canon,
                     "disposition": "absent this year"}
                )

    for prog, rows in per_program.items():
        rows.sort(key=lambda r: (r["fiscal_year"], r["source_file"],
                                 r["canonical_column"] or "zzz", r["source_column"]))
        with open(OUT / f"{prog}.csv", "w", newline="") as fh:
            w = csv.DictWriter(fh, fieldnames=[
                "fiscal_year", "source_file", "source_column",
                "canonical_column", "disposition"])
            w.writeheader()
            w.writerows(rows)
        ...
        print(f"{prog}: {n_map} mapped, {n_drop} dropped, {n_abs} absent-year cells")
