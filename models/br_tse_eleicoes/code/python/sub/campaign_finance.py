    ...
    for uf in _ufs_for_year(ano):
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "ds_tipo_bem_candidato": "tipo_item",
                "ds_bem_candidato": "descricao_item",
                "vr_bem_candidato": "valor_item",
            }
            df = select_named(df, keep_cols)
        elif ano <= 2012:
            # Headerless old-generation files (12 columns, the vintage the
            # Stata code parsed; pre-PR-#1564 mapping). Republished files
            # take the named path above.
...
def _build_receitas_2002() -> pd.DataFrame:
    ...
    if df.attrs.get("tse_has_header"):
        keep_cols = {
            ...
            "sg_uf": "sigla_uf",
            "sg_part": "sigla_partido",
            "ds_cargo": "cargo",
            "nr_cand": "numero_candidato",
            "dt_receita": "data_receita",
            "cd_cpf_cgc": "cpf_cnpj_doador",
            "sg_uf_doador": "sigla_uf_doador",
            "no_doador": "nome_doador",
            "vr_receita": "valor_receita",
            "tp_recurso": "especie_receita",
        }
        df = select_named(df, keep_cols)
    else:
        df = df[
...
def _build_receitas_2006() -> pd.DataFrame:
    ...
    if df.attrs.get("tse_has_header"):
        keep_cols = {
            ...
            "tipo_receita": "origem_receita",
            "descricao_tipo_recurso": "especie_receita",
            "nome_doador": "nome_doador",
            "numero_cpf_cgc_doador": "cpf_cnpj_doador",
            "unidade_eleitoral_doador": "sigla_uf_doador",
            "situacao_cadastral": "situacao_receita",
        }
        df = select_named(df, keep_cols)
    else:
        df = df[
    ...
    for uf in ufs:
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "cargo": "cargo",
                "entrega em conjunto?": "entrega_conjunto",
...
def _build_receitas_2014() -> pd.DataFrame:
    ...
    for uf in ufs:
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "nome do doador": "nome_doador",
                "nome do doador (receita federal)": "nome_doador_rf",
                "sigla ue doador": "sigla_uf_doador",
                "numero partido doador": "numero_partido_doador",
                "numero candidato doador": "numero_candidato_doador",
                "cod setor economico do doador": "cnae_2_doador",
                "setor economico do doador": "descricao_cnae_2_doador",
                ...
                "especie recurso": "natureza_receita",
                "descricao da receita": "descricao_receita",
                "cpf/cnpj do doador originario": "cpf_cnpj_doador_orig",
                "nome do doador originario": "nome_doador_orig",
                "tipo doador originario": "tipo_doador_orig",
                "setor economico do doador originario": "descricao_cnae_2_doador_orig",
                "nome do doador originario (receita federal)": "nome_doador_orig_rf",
            }
            df = select_named(df, keep_cols)
        else:
            df = df[
    ...
    try:
        ...
        if sup.attrs.get("tse_has_header"):
            keep_cols = {
                "cod. eleicao": "id_eleicao",
                "desc. eleicao": "tipo_eleicao",
                "cnpj prestador conta": "cnpj_prestador_contas",
            ...
            sup = select_named(sup, keep_cols)
        else:
            sup = sup[
                [
                    "v1",
    ...
    for uf in ufs:
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "dt_prestacao_contas": "data_prestacao_contas",
                "sq_prestador_contas": "sequencial_prestador_contas",
                "sg_uf": "sigla_uf",
                "sg_ue": "id_municipio_tse",
                "nr_cnpj_prestador_conta": "cnpj_prestador_contas",
                "ds_cargo": "cargo",
                "sq_candidato": "sequencial_candidato",
                "nr_candidato": "numero_candidato",
                ...
                "cd_cnae_doador": "cnae_2_doador",
                "ds_cnae_doador": "descricao_cnae_2_doador",
                "nr_cpf_cnpj_doador": "cpf_cnpj_doador",
                "nm_doador": "nome_doador",
                "nm_doador_rfb": "nome_doador_rf",
                "ds_esfera_partidaria_doador": "esfera_partidaria_doador",
                "sg_uf_doador": "sigla_uf_doador",
                "cd_municipio_doador": "id_municipio_tse_doador",
                "sq_candidato_doador": "sequencial_candidato_doador",
                "nr_candidato_doador": "numero_candidato_doador",
                "ds_cargo_candidato_doador": "cargo_candidato_doador",
                "nr_partido_doador": "numero_partido_doador",
                "sg_partido_doador": "sigla_partido_doador",
                "nr_recibo_doacao": "numero_recibo_doacao",
                "nr_documento_doacao": "numero_documento_doacao",
                "sq_receita": "sequencial_receita",
                "dt_receita": "data_receita",
...
def _build_despesas_2002() -> pd.DataFrame:
    ...
    if df.attrs.get("tse_has_header"):
        keep_cols = {
            ...
            "cd_cpf_cgc": "cpf_cnpj_fornecedor",
            "no_for": "nome_fornecedor",
    ...
    for uf in ufs:
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "descricao da despesa": "descricao_despesa",
            }
            df = select_named(df, keep_cols)
        else:
            df = df[
    ...
    for uf in ufs:
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "setor economico do fornecedor": "descricao_cnae_2_fornecedor",
                "data da despesa": "data_despesa",
                "valor despesa": "valor_despesa",
                "tipo despesa": "tipo_despesa",
                "descricao da despesa": "descricao_despesa",
    ...
    for uf in ufs:
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "nome do fornecedor": "nome_fornecedor",
                "nome do fornecedor (receita federal)": "nome_fornecedor_rf",
                "cod setor economico do fornecedor": "cnae_2_fornecedor",
                "setor economico do fornecedor": "descricao_cnae_2_fornecedor",
                "data da despesa": "data_despesa",
                "valor despesa": "valor_despesa",
                "tipo despesa": "tipo_despesa",
    ...
    for uf in ufs:
        ...
        if df.attrs.get("tse_has_header"):
            keep_cols = {
                ...
                "nr_partido": "numero_partido",
                "sg_partido": "sigla_partido",
                "ds_tipo_fornecedor": "tipo_fornecedor",
                "cd_cnae_fornecedor": "cnae_2_fornecedor",
                "ds_cnae_fornecedor": "descricao_cnae_2_fornecedor",
                "nr_cpf_cnpj_fornecedor": "cpf_cnpj_fornecedor",
                "nm_fornecedor": "nome_fornecedor",
                "nm_fornecedor_rfb": "nome_fornecedor_rf",
                "ds_esfera_part_fornecedor": "esfera_partidaria_fornecedor",
                "sg_uf_fornecedor": "sigla_uf_fornecedor",
                "cd_municipio_fornecedor": "id_municipio_tse_fornecedor",
                "sq_candidato_fornecedor": "sequencial_candidato_fornecedor",
                "nr_candidato_fornecedor": "numero_candidato_fornecedor",
                "ds_cargo_fornecedor": "cargo_fornecedor",
                "nr_partido_fornecedor": "numero_partido_fornecedor",
                "sg_partido_fornecedor": "sigla_partido_fornecedor",
                "ds_tipo_documento": "tipo_documento",
                "nr_documento": "numero_documento",
                "ds_origem_despesa": "origem_despesa",
                "sq_despesa": "sequencial_despesa",
                "dt_despesa": "data_despesa",
                "ds_despesa": "descricao_despesa",
