...
``pipelines/datasets/au_sa_ecsa_elections/schema.py`` is the single source of
truth. Everything downstream is generated so the three can never drift: the
architecture tables, one ``.sql`` model per table, and one ``schema.yml``.
...
# Directory foreign keys, written as the dbt ref the relationships test needs. The
# time directory's key is a STRUCT, so the field is addressed as ``ano.ano``; a
# bare ``ano`` binds to the struct and the test passes vacuously.
DIRECTORY_REFS = {
...
def write_schema_yml() -> None:
    ...
    for table, columns in TABLES.items():
        ...
        # A folded '>' scalar appends a trailing newline that the BigQuery
        # description then carries and the backend one does not; '>-' strips it.
