...
# Contexto do professor #ATG ----------------------------------------------
library(tidyverse)
library(haven)
library(readxl)
# ------------------------------------------------------------------------------
# Contexto do professor (Grade 4) - Processamento dos dados TIMSS 2023
# ------------------------------------------------------------------------------

# Define diretórios de entrada e saída
input_dir <-  "~/BD/timms/TIMSS2023_IDB_R_G4/TIMSS2023_IDB_R_G4/2_Data Files/R Data"
output_dir <- "~/BD/timms/timms_2023/tabelas/contexto_professor_g4"
...
# Lista todos os arquivos .RData de contexto do professor (que começam com 'ATG')
arquivos <- list.files(
  path = input_dir,
  pattern = "^atg.*\\.RData$",  # Expressão regular para arquivos .RData iniciando com 'ATG'
  ignore.case = TRUE,
  full.names = TRUE
)

# Lê a tabela de arquitetura com os nomes originais e novos das variáveis
arquitetura_contexto_professor <- readxl::read_excel(
  "~/BD/timms/timms_2023/arquitetura/[arquitetura]world_iae_timss_teacher_context_grade_4.xlsx"
)
arquitetura_contexto_professor <- arquitetura_contexto_professor %>%
  mutate(original_name = case_when(
    original_name == "ITLANG_CQ" ~ "ITLANG_TQ",
    original_name == "LCID_CQ" ~ "LCID_TQ",
    TRUE ~ original_name
  ))
# Define colunas de interesse com base na arquitetura
colunas <- arquitetura_contexto_professor$original_name
...
processar_arquivo_rdata <- function(caminho_arquivo, output_dir, colunas) {
  ...
  if (all(c("CTY", "IDCNTRY") %in% colnames(dados))) {
    dados$country_m49 <- ifelse(
      dados$CTY %in% c("COT", "CQU", "AAD", "ADU", "ATG"),
      NA,
      dados$IDCNTRY
    )
  } else {
...
world_iae_timss_teacher_context_grade_4 <- do.call(rbind, lista_dados)
...
nomes_antigos <- arquitetura_contexto_professor$original_name
nomes_novos   <- arquitetura_contexto_professor$name

# Filtra apenas os nomes existentes no dataframe
nomes_validos <- nomes_antigos[nomes_antigos %in% colnames(world_iae_timss_teacher_context_grade_4)]
nomes_correspondentes <- nomes_novos[nomes_antigos %in% colnames(world_iae_timss_teacher_context_grade_4)]

# Renomeia as colunas
world_iae_timss_teacher_context_grade_4 <- world_iae_timss_teacher_context_grade_4 %>%
...
ordem_desejada <- arquitetura_contexto_professor$name
ordem_final <- ordem_desejada[ordem_desejada %in% colnames(world_iae_timss_teacher_context_grade_4)]
...
guia_g4 <- read_excel("~/BD/timms/TIMSS2023_IDB_R_G4/TIMSS2023_IDB_R_G4/3_Supplemental Material/1_Codebook/T23_Codebook_G4.xlsx", sheet = "ATGM8")

# Se você quiser adicionar metadados do guia ao dataframe
df_final <- arquitetura_contexto_professor %>%
...
for (i in seq_len(nrow(df_final))) {
  ...
  if (!is.na(missing_val) && col_name %in% names(world_iae_timss_teacher_context_grade_4)) {
    world_iae_timss_teacher_context_grade_4[[col_name]] <- ifelse(
      world_iae_timss_teacher_context_grade_4[[col_name]] == missing_val,
      NA,
      world_iae_timss_teacher_context_grade_4[[col_name]]
    )
  }
}

write.csv(world_iae_timss_teacher_context_grade_4, "~/BD/timms/timms_2023/tabelas/grade_4/world_iae_timss_teacher_context_grade_4.csv", row.names = FALSE)
