...
arquitetura_contexto_prof_sci <- readxl::read_excel(
  "~/BD/timms/timms_2023/arquitetura/[arquitetura]world_iae_timss_teacher_science_grade_8.xlsx")
...
world_iae_timss_teacher_science_grade_8 <- do.call(rbind, lista_dados)
...
nomes_validos <- nomes_antigos[nomes_antigos %in% colnames(world_iae_timss_teacher_science_grade_8)]
nomes_correspondentes <- nomes_novos[nomes_antigos %in% colnames(world_iae_timss_teacher_science_grade_8)]

# Renomeia as colunas
world_iae_timss_teacher_science_grade_8 <- world_iae_timss_teacher_science_grade_8 %>%
...
ordem_final <- ordem_desejada[ordem_desejada %in% colnames(world_iae_timss_teacher_science_grade_8)]
...
for (i in seq_len(nrow(df_final))) {
  ...
  if (!is.na(missing_val) && col_name %in% names(world_iae_timss_teacher_science_grade_8)) {
    world_iae_timss_teacher_science_grade_8[[col_name]] <- ifelse(
      world_iae_timss_teacher_science_grade_8[[col_name]] == missing_val,
      NA,
      world_iae_timss_teacher_science_grade_8[[col_name]]
    )
  }
}

write.csv(world_iae_timss_teacher_science_grade_8, "world_iae_timss_teacher_science_grade_8.csv", 
          row.names = FALSE, 
          na = "", 
          fileEncoding = "UTF-8",
          quote = TRUE)