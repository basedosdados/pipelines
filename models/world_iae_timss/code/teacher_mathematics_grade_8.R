# Contexto da escola #BTM ----------------------------------------------
library(dplyr)    
library(readxl)  
library(haven) 

# ------------------------------------------------------------------------------
# Contexto do professor de matemática (Grade 8) - Processamento dos dados TIMSS 2023
# ------------------------------------------------------------------------------

# Define diretórios de entrada e saída
input_dir <- "~/BD/timms/TIMSS2023_IDB_R_G8/TIMSS2023_IDB_R_G8/2_Data Files/R Data"
output_dir <- "~/BD/timms/timms_2023/tabelas/contexto_prof_matematica_g8"
...
#Lista todos os arquivos .RData de contexto do aluno (que começam com 'btm')
arquivos <- list.files(
  path = input_dir,
  pattern = "^btm.*\\.RData$",  # Expressão regular para arquivos .RData iniciando com 'btm'
  ignore.case = TRUE,
  full.names = TRUE
)
...
nomes_antigos <- arquitetura_contexto_prof_mat$original_name
...
df_final <- arquitetura_contexto_prof_mat %>%
