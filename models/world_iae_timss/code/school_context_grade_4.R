...
# Contexto da escola #ACG ----------------------------------------------

# ------------------------------------------------------------------------------
# Contexto da Casa do Estudante (Grade 8) - Processamento dos dados TIMSS 2023
# ------------------------------------------------------------------------------

# Define diretórios de entrada e saída
input_dir <-  "~/BD/timms/TIMSS2023_IDB_R_G4/TIMSS2023_IDB_R_G4/2_Data Files/R Data"
output_dir <- "~/BD/timms/timms_2023/tabelas/contexto_escola"
...
# Lista todos os arquivos .RData de contexto de casa do aluno (que começam com 'ACG')
arquivos <- list.files(
  path = input_dir,
  pattern = "^ACG.*\\.RData$",  # Expressão regular para arquivos .RData iniciando com 'ACG'
  ignore.case = TRUE,
  full.names = TRUE
)

# Lê a tabela de arquitetura com os nomes originais e novos das variáveis
arquitetura_school_context <- readxl::read_excel(
  "~/BD/timms/timms_2023/arquitetura/[arquitetura]world_iae_timss_school_context_grade_4.xlsx"
)

# Define colunas de interesse com base na arquitetura
colunas <- arquitetura_school_context$original_name
...
processar_arquivo_rdata <- function(caminho_arquivo, output_dir, colunas) {
  ...
  if (all(c("CTY", "IDCNTRY") %in% colnames(dados))) {
    dados$country_m49 <- ifelse(
      dados$CTY %in% c("COT", "CQU", "AAD", "ADU", "ACG"),
      NA,
      dados$IDCNTRY
    )
  } else {
...
# Combina todos os dataframes em um único dataframe final
  world_iae_timss_school_context_grade_4 <- do.call(rbind, lista_dados)
...
nomes_antigos <- arquitetura_school_context$original_name
nomes_novos   <- arquitetura_school_context$name

# Filtra apenas os nomes existentes no dataframe
nomes_validos <- nomes_antigos[nomes_antigos %in% colnames(world_iae_timss_school_context_grade_4)]
nomes_correspondentes <- nomes_novos[nomes_antigos %in% colnames(world_iae_timss_school_context_grade_4)]

# Renomeia as colunas
world_iae_timss_school_context_grade_4 <- world_iae_timss_school_context_grade_4 %>%
...
ordem_desejada <- arquitetura_school_context$name
ordem_final <- ordem_desejada[ordem_desejada %in% colnames(world_iae_timss_school_context_grade_4)]
...
guia_g4 <- read_excel("~/BD/timms/TIMSS2023_IDB_R_G4/3_Supplemental Material/1_Codebook/T23_Codebook_G4.xlsx", sheet = "ACGM8")

# Se você quiser adicionar metadados do guia ao dataframe
df_final <- arquitetura_school_context %>%
...
for (i in seq_len(nrow(df_final))) {
  ...
  if (!is.na(missing_val) && col_name %in% names(world_iae_timss_school_context_grade_4)) {
    world_iae_timss_school_context_grade_4[[col_name]] <- ifelse(
      world_iae_timss_school_context_grade_4[[col_name]] == missing_val,
      NA,
      world_iae_timss_school_context_grade_4[[col_name]]
    )
  }
}

write.csv(world_iae_timss_school_context_grade_4, "~/BD/timms/timms_2023/final_data/grade_4/world_iae_timss_school_context_grade_4.csv", row.names = FALSE)
