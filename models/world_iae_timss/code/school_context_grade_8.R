...
# ------------------------------------------------------------------------------
# Contexto da Escola (Grade 8) - Processamento dos dados TIMSS 2023
# ------------------------------------------------------------------------------

# Define diretórios de entrada e saída
input_dir <- "~/BD/timms/TIMSS2023_IDB_R_G8/TIMSS2023_IDB_R_G8/2_Data Files/R Data"
output_dir <- "~/BD/timms/timms_2023/tabelas/contexto_escola_g8"
...
#Lista todos os arquivos .RData de contexto do aluno (que começam com 'bcg')
arquivos <- list.files(
  path = input_dir,
  pattern = "^bcg.*\\.RData$",  # Expressão regular para arquivos .RData iniciando com 'bsg'
  ignore.case = TRUE,
  full.names = TRUE
)

# Lê a tabela de arquitetura com os nomes originais e novos das variáveis
arquitetura_contexto_escola <- readxl::read_excel(
  "~/BD/timms/timms_2023/arquitetura/[arquitetura]world_iae_timss_school_context_grade_8.xlsx")

# Define colunas de interesse com base na arquitetura
colunas <- arquitetura_contexto_escola$original_name
...
world_iae_timss_school_context_grade_8 <- do.call(rbind, lista_dados)
...
nomes_antigos <- arquitetura_contexto_escola$original_name
nomes_novos   <- arquitetura_contexto_escola$name

# Filtra apenas os nomes existentes no dataframe
nomes_validos <- nomes_antigos[nomes_antigos %in% colnames(world_iae_timss_school_context_grade_8)]
nomes_correspondentes <- nomes_novos[nomes_antigos %in% colnames(world_iae_timss_school_context_grade_8)]

# Renomeia as colunas
world_iae_timss_school_context_grade_8 <- world_iae_timss_school_context_grade_8 %>%
...
ordem_desejada <- arquitetura_contexto_escola$name
ordem_final <- ordem_desejada[ordem_desejada %in% colnames(world_iae_timss_school_context_grade_8)]
...
guia_g8 <- read_excel("~/BD/timms/TIMSS2023_IDB_R_G8/TIMSS2023_IDB_R_G8/3_Supplemental Material/1_Codebook/T23_Codebook_G8.xlsx", sheet = "BCGM8")

# Se você quiser adicionar metadados do guia ao dataframe
df_final <- arquitetura_contexto_escola %>%
...
for (i in seq_len(nrow(df_final))) {
  ...
  if (!is.na(missing_val) && col_name %in% names(world_iae_timss_school_context_grade_8)) {
    world_iae_timss_school_context_grade_8[[col_name]] <- ifelse(
      world_iae_timss_school_context_grade_8[[col_name]] == missing_val,
      NA,
      world_iae_timss_school_context_grade_8[[col_name]]
    )
  }
}

write.csv( world_iae_timss_school_context_grade_8, "~/BD/timms/timms_2023/final_data/world_iae_timss_school_context_grade_8.csv", row.names = FALSE)
