"""Generate the au_tas_tec_elections architecture CSVs and the columns JSON.
...
# The observation translations are maintained separately from the column specs. Read
# them if the schema module carries them and fall back to no translation otherwise, so
# regenerating the architecture never depends on that table being complete.
OBSERVATION_TRANSLATIONS: dict[str, tuple[str | None, str | None]] = getattr(
    schema, "OBSERVATION_TRANSLATIONS", {}
)


def _observation(text: str, index: int) -> str | None:
    if not text:
        return None
    return OBSERVATION_TRANSLATIONS.get(text, (None, None))[index]
        ...
        columns_json[table] = [
            {
                ...
                "observations_en": _observation(c.observations, 0),
                "observations_es": _observation(c.observations, 1),
