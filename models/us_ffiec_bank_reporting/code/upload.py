"""Upload the cleaned parquet of us_ffiec_bank_reporting to BigQuery dev staging.

    uv run python models/us_ffiec_bank_reporting/code/upload.py            # every table
    uv run python models/us_ffiec_bank_reporting/code/upload.py financials # one table
...
* `bd.Table.create` infers the schema by reading the FIRST file in the prefix
  into pandas.  `call_report_item` runs to hundreds of millions of rows, so a
  real partition would blow up memory here and again in the table-approve
  action.  A 0-row `00_header.parquet` is written at the table root, which sorts
  ahead of every `year=...` partition and costs nothing to read.
...
OUT = (
    Path(
        os.environ.get(
            "FFIEC_DATA_DIR",
            Path.home() / "Downloads/us_ffiec_bank_reporting_data",
        )
    )
    / "output"
)
...
UNPARTITIONED = {"mdrm_item", "dictionary"}
