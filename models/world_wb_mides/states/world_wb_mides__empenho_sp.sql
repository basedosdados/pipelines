-- São Paulo (SP) contribution to world_wb_mides.empenho.
...
with
    ...
    anulacao as (
        ...
        from {{ set_datalake_project("world_wb_mides_staging.raw_despesa_sp") }} a
        left join
            {{ set_datalake_project("world_wb_mides_staging.aux_municipio_sp") }} m
    ...
    reforco as (
        ...
        from {{ set_datalake_project("world_wb_mides_staging.raw_despesa_sp") }} r
        left join
            {{ set_datalake_project("world_wb_mides_staging.aux_municipio_sp") }} m
    ...
    empenho_municipio_sp as (
        select
            ...
            safe_cast(
                left(replace(tipo_empenho, 'Por Estimativa', 'Estimativo'), 1) as string
            ) as modalidade,
            ...
            round(safe_cast(0 as float64), 2) as valor_ajuste,
            round(
                safe_cast(valor_empenho as float64) - safe_cast(cancelado as float64), 2
            ) as valor_final,
        from
            {{
