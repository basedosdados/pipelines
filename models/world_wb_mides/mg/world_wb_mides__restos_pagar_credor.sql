...
select
    ...
    safe_cast(
        concat(
            p_restos_pagar.id_restos_pagar_bd, ' ', ifnull(t.num_doc_credor, '')
        ) as string
    ) as id_restos_pagar_credor_bd,
    safe_cast(p_restos_pagar.id_restos_pagar_bd as string) as id_restos_pagar_bd,
    safe_cast(t.seq_credor_rsp as string) as id_credor_rsp,
...
from
    {{ set_datalake_project("world_wb_mides_staging.raw_restos_pagar_credor_mg") }} as t
