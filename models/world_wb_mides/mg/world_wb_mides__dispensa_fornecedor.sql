...
select
    ...
    -- sequence. `seq_forn_dispensa` is appended so the key identifies a row, at the
    ...
    safe_cast(
        concat(
            ...
            ' ',
            ifnull(t.num_doc_fornecedor, ''),
            ' ',
            ifnull(t.seq_forn_dispensa, '')
        ) as string
    ) as id_dispensa_fornecedor_bd,
    safe_cast(p_dispensa.id_dispensa_bd as string) as id_dispensa_bd,
    safe_cast(t.seq_forn_dispensa as string) as id_forn_dispensa,
    ...
    safe_cast(t.num_doc_fornecedor as string) as numero_doc_fornecedor,
    safe_cast(t.dsc_nom_fornecedor as string) as nom_fornecedor,
    safe_cast(t.num_inscr_estadual as string) as numero_inscr_estadual,
    safe_cast(t.dsc_sigla_uf as string) as sigla_uf_fornecedor,
    ...
    safe_cast(t.num_cndt as string) as numero_cndt,
    safe_cast(t.data_emi_cndt as date) as data_emi_cndt,
    safe_cast(t.data_val_cndt as date) as data_val_cndt,
    safe_cast(t.num_quant_item as string) as numero_quant_item,
    safe_cast(t.valor_item as float64) as valor_item
from
    {{ set_datalake_project("world_wb_mides_staging.raw_dispensa_fornecedor_mg") }} as t
