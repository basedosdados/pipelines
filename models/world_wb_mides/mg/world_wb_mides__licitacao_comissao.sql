...
select
    ...
    -- sequence. `seq_com_licitacao` is appended so the key identifies a row, at the
    ...
    safe_cast(
        concat(
            ...
            ' ',
            ifnull(t.seq_com_licitacao, '')
        ) as string
    ) as id_licitacao_comissao_bd,
    safe_cast(p_licitacao.id_licitacao_bd as string) as id_licitacao_bd,
    safe_cast(t.seq_com_licitacao as string) as id_com_licitacao,
    safe_cast(t.seq_licitacao as string) as id_licitacao,
    safe_cast(t.orgao as string) as orgao,
    safe_cast(t.dsc_tipo_comissao as string) as tipo_comissao,
    safe_cast(t.dsc_ato_nomeacao as string) as ato_nomeacao,
    safe_cast(t.num_ato_nomeacao as string) as numero_ato_nomeacao,
    safe_cast(t.data_ato_nomeacao as date) as data_ato_nomeacao,
    safe_cast(t.data_ini_vigencia as date) as data_ini_vigencia,
    safe_cast(t.data_fim_vigencia as date) as data_fim_vigencia,
    safe_cast(t.dsc_tipo_atrib as string) as tipo_atrib,
    safe_cast(t.dsc_cargo as string) as cargo,
    safe_cast(t.dsc_tipo_nat_cargo as string) as tipo_nat_cargo,
    safe_cast(t.num_documento as string) as numero_documento,
    safe_cast(t.nom_pessoa as string) as nome_pessoa
from {{ set_datalake_project("world_wb_mides_staging.raw_licitacao_comissao_mg") }} as t
