...
select
    ...
    -- sequence. `seq_hom_licitacao` is appended so the key identifies a row, at the
    ...
    safe_cast(
        concat(
            ...
            ' ',
            ifnull(t.seq_hom_licitacao, '')
        ) as string
    ) as id_licitacao_homologacao_bd,
    safe_cast(p_licitacao_item.id_licitacao_item_bd as string) as id_licitacao_item_bd,
    safe_cast(t.seq_hom_licitacao as string) as id_hom_licitacao,
...
from
    {{ set_datalake_project("world_wb_mides_staging.raw_licitacao_homologacao_mg") }}
    as t
