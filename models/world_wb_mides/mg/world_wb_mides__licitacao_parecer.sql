...
select
    ...
    -- sequence. `seq_parecer_licit` is appended so the key identifies a row, at the
    ...
    safe_cast(
        concat(
            ...
            ' ',
            ifnull(t.num_doc_resp_parecer, ''),
            ' ',
            ifnull(t.data_parecer, ''),
            ' ',
            ifnull(t.seq_parecer_licit, '')
        ) as string
    ) as id_licitacao_parecer_bd,
    safe_cast(p_licitacao.id_licitacao_bd as string) as id_licitacao_bd,
    safe_cast(t.seq_parecer_licit as string) as id_parecer_licit,
    safe_cast(t.seq_licitacao as string) as id_licitacao,
    safe_cast(t.orgao as string) as orgao,
    safe_cast(t.data_parecer as date) as data_parecer,
    safe_cast(t.dsc_tipo_parecer as string) as tipo_parecer,
    safe_cast(t.num_doc_resp_parecer as string) as numero_doc_resp_parecer,
    safe_cast(t.nom_resp_parecer as string) as nome_resp_parecer
from {{ set_datalake_project("world_wb_mides_staging.raw_licitacao_parecer_mg") }} as t
