...
| Step | State |
|---|---|
| 1–2 context, architecture | done — `code/architecture/*.csv`, 12 tables |
| 3–4 download, clean | done — 3,249,324 rows, counts match the source exactly |
| 5 upload to `basedosdados-dev` | done — all 12 staging tables, row counts verified |
| 6 dbt models + `schema.yml` | done — generated by `code/build_dbt.py` |
| 6b auxiliary files | done — bundle in `gs://basedosdados/auxiliary_files/us_fhfa_hpi/<table>/` for the four master tables; **every published URL returns HTTP 400 anonymously** (the known requester-pays bug affecting all 84 production tables using the field) |
...
`quarterly_state.state_abbreviation` cannot store its directory link:
`Column.directoryPrimaryKey` only accepts a directory column flagged
`is_primary_key`, and `br_bd_diretorios_us.state`'s key is `id_state`, not
`abbreviation`. All 52 values do resolve against the directory, so the dbt
`relationships` test is kept; only the backend metadata cannot express it.
