"""Upload the cleaned us_fhfa_hpi parquet tables to BigQuery.

Usage:
    uv run python models/us_fhfa_hpi/code/upload.py [--env dev|prod] [table_slug ...]
...
Reads the parquet written by clean_data.py under ``~/Downloads/us_fhfa_hpi_data``
(override with ``FHFA_HPI_DATA_DIR``).
"""
...
# (table_slug, expected_rows) — smallest first, counts from clean_data.py
TABLES = [
    ("dicionario", 18),
    ("annual_national", 51),
    ("annual_state", 2_601),
    ("monthly_national", 4_260),
    ("quarterly_national", 5_112),
    ("quarterly_state", 31_159),
    ("annual_zip3", 42_050),
    ("annual_cbsa", 43_766),
    ("annual_county", 106_252),
    ("quarterly_metro", 145_480),
    ("annual_zip5", 689_533),
    ("annual_tract", 2_179_042),
]
