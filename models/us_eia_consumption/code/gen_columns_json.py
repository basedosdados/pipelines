"""Write the bulk_upsert_columns payload for each table, from the architecture CSV.

The backend metadata is generated from the same CSV as the transform and the dbt
model, so the three cannot describe different schemas.

Run: uv run python models/us_eia_seds/code/gen_columns_json.py [out_dir]
"""
...
DICIONARIO_COLUMNS = [
    ...
    },
    {
        ...
        "bigquery_type": "STRING",
        "description_pt": "Anos em que o valor aparece, na notação início(intervalo)fim",
        "description_en": "Years over which the value appears, in start(interval)end notation",
        "description_es": "Años en que aparece el valor, en la notación inicio(intervalo)fin",
    },
    {
        "name": "valor",
        "bigquery_type": "STRING",
