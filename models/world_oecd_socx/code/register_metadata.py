...
end (``--publish``), because the staging frontend is not the public site.

Adapted from ``world_oecd_education/code/register_metadata.py``; SOCX is a single
table (``expenditure``) plus the ``dicionario``.
"""
...
# Content tags only. Area (countries) and organisation (OECD) are separate
# metadata, and a tag that merely restates the theme adds nothing. Each entry is
# a list of candidate slugs, most-preferred first, because the backends do not
# share a tag vocabulary (prod English kebab-case, staging legacy Portuguese).
# The first slug that exists wins; a concept with no match is dropped rather than
# inventing a near-duplicate.
TAG_CANDIDATES = [
    ["social-expenditure", "gasto-social"],
    ["social-policy", "politica-social"],
    ["welfare", "bem-estar"],
    ["public-spending", "spending", "gasto"],
    ["pension", "previdencia"],
    ["health", "saude"],
...
# The dataset's subject is social policy / public economics. Try the most
# specific existing theme first; drop silently if none match rather than create.
THEME_CANDIDATES = ["economics", "society", "social", "economy"]


def resolve_first(candidates, vocabulary):
    """First candidate slug that exists in ``vocabulary`` -> its id, else None."""
...
def resolve_tags(vocabulary):
    ...
    for candidates in TAG_CANDIDATES:
        tid = resolve_first(candidates, vocabulary)
        if tid:
            out.append(tid)
    return out


NAME_PT = "OCDE: gasto social"
NAME_EN = "OECD: social expenditure"
NAME_ES = "OCDE: gasto social"

DESC_PT = (
    "Base agregada do OECD Social Expenditure Database (SOCX): gasto social público "
    "e privado por área de política (velhice, sobreviventes, incapacidade, saúde, "
    "família, programas de mercado de trabalho, desemprego, habitação e outras), por "
    "origem do financiamento (público, privado obrigatório e privado voluntário) e "
    "natureza da prestação (em dinheiro ou em espécie), para países da OCDE e "
    "parceiros desde 1980. Cada valor é expresso como proporção do PIB, do gasto "
    "público total, em dólares por habitante em PPC ou em moeda nacional. A tabela é "
    "um cubo longo: uma observação por combinação de dimensões, com o código de cada "
    "dimensão resolvido na tabela dicionario."
)
DESC_EN = (
    "Aggregated data of the OECD Social Expenditure Database (SOCX): public and "
    "private social expenditure by policy area (old age, survivors, incapacity, "
    "health, family, active labour market programmes, unemployment, housing and "
    "other), by source of funding (public, mandatory private and voluntary private) "
    "and nature of the benefit (cash or in kind), for OECD and partner countries "
    "since 1980. Each value is expressed as a share of GDP, of total government "
    "expenditure, in US dollars per head at PPP or in national currency. The table "
    "is a long cube: one observation per combination of dimensions, with each "
    "dimension's code resolved in the dicionario table."
)
DESC_ES = (
    "Datos agregados de la OECD Social Expenditure Database (SOCX): gasto social "
    "público y privado por área de política (vejez, sobrevivientes, incapacidad, "
    "salud, familia, programas del mercado laboral, desempleo, vivienda y otras), "
    "por origen del financiamiento (público, privado obligatorio y privado "
    "voluntario) y naturaleza de la prestación (en efectivo o en especie), para "
    "países de la OCDE y socios desde 1980. Cada valor se expresa como proporción "
    "del PIB, del gasto público total, en dólares por habitante en PPA o en moneda "
    "nacional. La tabla es un cubo largo: una observación por combinación de "
...
RAW_SOURCE = dict(
    ...
    description_pt=(
        ...
        "estatísticas. A base SOCX agregada vem do dataflow DF_SOCX_AGG, que "
        "carrega o cubo inteiro; os demais dataflows de gasto social são recortes "
        "dele."
    ),
    description_en=(
        "The SDMX REST interface through which the OECD publishes all of its "
        "statistical databases. The SOCX Aggregated database comes from the "
        "DF_SOCX_AGG dataflow carrying the whole cube; the other social-expenditure "
        "dataflows are views over it."
    ),
    description_es=(
        "Interfaz SDMX REST por la que la OCDE publica todas sus bases estadísticas. "
        "La base SOCX agregada proviene del dataflow DF_SOCX_AGG, que contiene el "
        "cubo completo; los demás dataflows de gasto social son recortes de él."
    ),
)

# Grain: SOCX is a country-year panel.
OBSERVATION_LEVELS = {slug: ["country", "year"] for slug in TABLES}
...
def main():
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "flip the dataset to published. Safe on staging; on prod, only after "
            "the PR has merged, table-approve has materialised the tables, and they "
            "have been verified."
        ),
    )
    args = ap.parse_args()
    ...
    theme_id = resolve_first(THEME_CANDIDATES, ids["theme"])
    if not theme_id:
        raise SystemExit(
            f"no theme among {THEME_CANDIDATES} exists in {env}: "
            f"{sorted(ids['theme'])[:20]}"
        )
    tag_ids = resolve_tags(ids["tag"])
    print(f"theme -> {theme_id}; {len(tag_ids)} tags resolved")
