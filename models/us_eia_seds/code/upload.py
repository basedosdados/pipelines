...
native load-job table would silently ignore every file a later pipeline run
writes). ``dump_mode="append"`` because ``"overwrite"`` drops the production
table too, even against dev.
...
# seds_consumption is hive-partitioned by year (1960-2024 => 65 partitions);
# dicionario is a single file.
MIN_FILES = {"seds_consumption": 60, "dicionario": 1}
...
def _patched_bucket(
    ...
    """Return a bucket handle pinned to the billing project (buckets are requester-pays)."""
