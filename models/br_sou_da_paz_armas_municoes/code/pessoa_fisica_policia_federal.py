...
def pessoa_fisica_policia_federal(
    ...
    df.to_csv(
        constants.tabelas.value["pessoa_fisica_policia_federal"]["save_table"],
        sep=",",
        encoding="utf-8",
        index=False,
    )
...
if __name__ == "__main__":
    ...
    tb = bd.Table(
        ...
        table_id="pessoa_fisica_policia_federal",
        bucket_name="basedosdados-consultoria",
        mode="sou_da_paz",
    )

    tb.create(
        path=constants.tabelas.value["pessoa_fisica_policia_federal"][
            "save_table"
        ],
