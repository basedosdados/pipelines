"""Upload cleaned au_abs_household_income_wealth parquet tables to BigQuery.

Usage:
    uv run python models/au_abs_household_income_wealth/code/upload.py [--env dev|prod] [table ...]
...
Three tables, all under the output directory (default
~/Downloads/au_abs_household_income_wealth_data/output, overridable with
--output):
    household_estimate            hive-partitioned by year=YYYY
    experimental_wealth_estimate  hive-partitioned by year=YYYY
    dictionary                    one parquet file, unpartitioned

Pass each table's root directory to bd.Table.create, never a single partition:
handing it "year=2020" lands the object at the prefix root and the hive key is
lost.
"""
...
DATASET_ID = "au_abs_household_income_wealth"
if "--output" in _argv:
    _i = _argv.index("--output")
    OUTPUT_ROOT = Path(_argv[_i + 1]).expanduser()
    _argv = _argv[:_i] + _argv[_i + 2 :]
else:
    OUTPUT_ROOT = (
        Path.home()
