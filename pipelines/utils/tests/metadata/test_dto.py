...
def test_datetimerange_interval_defaults_to_one_and_is_emitted() -> None:
    """O default 1 sai no payload — condição para o CREATE.

    O backend exige `interval` ao CRIAR um range com início e fim; sem ele o
    upsert só conseguia UPDATE. Como não é None, sobrevive ao
...
def test_datetimerange_interval_respects_explicit_value() -> None:
    """Um interval explícito (ex.: 2, série bienal) é preservado, não fixado."""
    dto = DateTimeRangeInput(coverage=UUID, endYear=2026, interval=3)
    assert dto.interval == 3


@pytest.mark.parametrize("bad", [0, -1])
def test_datetimerange_interval_must_be_positive(bad: int) -> None:
    """interval é `ge=1`: zero ou negativo é rejeitado."""
    with pytest.raises(ValidationError):
