...
# ! ================================== CGU - Receitas Públicas =====================================

with Flow(name="CGU - Receitas Públicas", code_owners=["trick"]) as flow_cgu_receitas_publicas:

    dataset_id = Parameter(
        "dataset_id", default="br_cgu_receitas_publicas", required=True
    )
    table_id = Parameter(
        "table_id", required=True
    )
...
flow_cgu_receitas_publicas.storage = GCS(constants.GCS_FLOWS_BUCKET.value)
flow_cgu_receitas_publicas.run_config = KubernetesRun(image=constants.DOCKER_IMAGE.value)
