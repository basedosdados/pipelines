...
from pipelines.crawler.ibge_inflacao.utils import get_legacy_session
...
@task
def collect_data(dataset_id: str, table_id: str, periodo: str) -> None:
    asyncio.run(
        collect_data(
            dataset_id=dataset_id,
            table_id=table_id,
            aggregates=constants.DATASETS[dataset_id]["aggregates"],
            variables=constants.DATASETS[dataset_id]["variables"],
            periods=periodo,
            geo_level=constants.GEO_LEVELS[table_id],
        )
    )


@task
def json_to_csv(table_id: str, dataset_id: str, output: str, periodo: str):
    input = os.path.join(
    ...
    output_path = os.path.join(output, dataset_id, table_id)
    to_partitions(
        data=df,
