"""
Flows for au_apra_superannuation — Prefect 3.

The source is one APRA workbook, "Quarterly superannuation performance
statistics", republished every quarter with the full history embedded. A single
flow downloads it once and rebuilds all four tables. Because every release ships
the whole back-series, each run is a **full replace**, not an incremental
append (see ``_DUMP_MODE`` below for why that is spelled ``"append"``).
...
Deploy: ``.github/scripts/deploy_flows.py`` auto-discovers ``au_apra_superannuation_flow``;
...
@flow(name="au_apra_superannuation", log_prints=True)
def au_apra_superannuation_flow(
    materialize_to_prod: bool = True,
    update_metadata: bool = True,
    force_run: bool = False,
) -> None:
    """Download the current APRA superannuation workbook, rebuild all tables.
    ...
    work_dir = tempfile.mkdtemp(prefix="au_apra_superannuation_")
    try:
        input_dir = download_superannuation(work_dir=work_dir)
        result = clean_superannuation(work_dir=work_dir, input_dir=input_dir)
...
# APRA releases the quarterly superannuation performance statistics roughly 8
