...
def IGP_DI_anual():
    ...
    igpdi_anual = reduce(lambda  left,right: pd.merge(left,right,on=['ano'],
                                                how='outer'), dfs)
    igpdi_anual.to_csv('/br_fgv_igp/igpdi_ano.csv', encoding='utf-8', na_rep='', index=False)
...
def IGP_DI_mensal():
    ...
    igpdi_mensal = reduce(lambda  left,right: pd.merge(left,right,on=['ano', 'mes'],
                                            how='outer'), dfs)
    igpdi_mensal.to_csv('/br_fgv_igp/igpdi_mes.csv', encoding='utf-8', na_rep='', index=False)
...
def IGP_M_mensal():
    ...
    rename = {'YEAR':'ano', 'MONTH':'mes', 'VALUE ((% a.m.))':'variacao_primeiro_decendio'}
    ...
    rename = {'YEAR':'ano', 'MONTH':'mes', 'VALUE ((% a.m.))':'variacao_segundo_decendio'}
    var_segundo_decendio.rename(columns=rename, inplace=True)

    dfs = [indice, var_mensal, var_primeiro_decendio, var_segundo_decendio]

    igpm_mensal = reduce(lambda  left,right: pd.merge(left,right,on=['ano', 'mes'],
                                            how='outer'), dfs)
    igpm_mensal.to_csv('/br_fgv_igp/igpm_mes.csv', encoding='utf-8', na_rep='', index=False)
...
def IGP_OG_mensal():
    ...
    igpog_mensal = reduce(lambda  left,right: pd.merge(left,right,on=['ano', 'mes'],
                                            how='outer'), dfs)
    igpog_mensal.to_csv('/br_fgv_igp/igpog_mes.csv', encoding='utf-8', na_rep='', index=False)
