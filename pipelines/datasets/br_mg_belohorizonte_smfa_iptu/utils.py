...
def replace_variables(df: pd.DataFrame) -> pd.DataFrame:
    bool = ['indicador_rede_telefonica',
            'indicador_meio_fio',
    ...
    for valores_bool in bool:
            df[valores_bool] = df[valores_bool].replace("SIM", 'true').replace("NÃO", "false")

    inicap = ['tipo_logradouro',
            'logradouro',
            'tipo_construtivo',
            'tipo_ocupacao',
            'tipologia',
    ...
    for variaveis_inicap in inicap:
        ...
        df['tipo_logradouro'] = df['tipo_logradouro'].replace('Ave', 'Avenida').replace('Est', 'Estrada').replace('Bec', 'Beco').replace('Rod', 'Rodovia').replace('Trv', 'Trevo').replace('Ala', 'Alameda').replace('Pca', 'Praça')

        return df

def new_columns_endereco(df: pd.DataFrame) -> pd.DataFrame:
    df['endereco'] = (df['tipo_logradouro'].fillna("") + ' ' + df['logradouro'].fillna("") + ',' + ' ' + df['numero_imovel'].fillna(""))
