...
def get_data_source_max_date() -> datetime.date:
    """
    ...
    max_folder_date = data_url(url=url, headers=headers)
...
def main(
    ...
    for tabela in tabelas:
        ...
        input_path = f"/tmp/data/br_me_cnpj/input/data={max_folder_date}/"
        os.makedirs(input_path, exist_ok=True)
        log("Pasta destino input construído")

        # Define o caminho para a pasta de saída (output)
        output_path = destino_output(sufixo, max_folder_date)

        # Loop para baixar e processar os arquivos
        for i in range(0, 10):
            if tabela != "Simples":
                nome_arquivo = f"{tabela}{i}"
                url_download = f"https://arquivos.receitafederal.gov.br/cnpj/dados_abertos_cnpj/{max_folder_date.strftime('%Y-%m')}/{tabela}{i}.zip"
                ...
                url_download = f"https://arquivos.receitafederal.gov.br/cnpj/dados_abertos_cnpj/{max_folder_date.strftime('%Y-%m')}/{tabela}.zip"
                if nome_arquivo not in arquivos_baixados:
                    arquivos_baixados.append(nome_arquivo)
                    asyncio.run((download_unzip_csv(url_download, input_path)))
                    process_csv_simples(
