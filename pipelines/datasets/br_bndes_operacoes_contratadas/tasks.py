...
from pipelines.datasets.br_bndes_operacoes_contratadas.utils import download_xlsx, get_xlsx_metadata
from pipelines.datasets.br_bndes_operacoes_contratadas.constants import constants
...
def process_data(
    ...
    # Cnaes
    df_operacoes = df_operacoes.rename(columns=constants.DATA_RENAMMING_MAPPING.value)
    df_operacoes["secao_cnae"] = df_operacoes["cnae_2"].str.extract(r"(^[A-Z]{1})")
    df_operacoes["subclasse_cnae"] = df_operacoes["cnae_2"].str.extract(r"(\d+$)")
    ...
    df_operacoes.loc[(df_operacoes["id_municipio"]==9999999)|
                    (df_operacoes["id_municipio"]==0), "id_municipio"] = None
    df_operacoes["id_municipio"] = df_operacoes["id_municipio"].astype("str").str.replace(".0","")
    
    # indicador_inovacao mapping to 1, 0
    df_operacoes["indicador_inovacao"] = df_operacoes["indicador_inovacao"]\
    ...
    if isinstance(data_apuracao, datetime.datetime):
        ...
        try:
            df_operacoes["data_apuracao"] = datetime.datetime.strptime(data_apuracao, "%d/%m/%Y")
        except ValueError:
            df_operacoes["data_apuracao"] = datetime.datetime.strptime(data_apuracao, "%d/%m/%y")
...
def process_de_para_cnae(
    ...
    df_cnae = df_cnae.rename(columns=constants.DE_PARA_CNAE_RENAME_MAPPING.value)
    ...
    df_cnae[["limite_inferior", "limite_superior"]] = df_cnae["lista_cnaes_2"].str.split(r"\(restante\)| a ").apply(pd.Series)
    ...
    df_cnae_listas["lista_cnaes_2"] = df_cnae_listas["lista_cnaes_2"].str.split(r"[\n\s]*e{1}[\n\s]*|[\n\s]*,{1}[\n\s]*")
    df_cnae_listas = df_cnae_listas.explode("lista_cnaes_2")
    df_cnae_listas["secao"] = df_cnae_listas["lista_cnaes_2"].str.extract(r"(^[A-Z]{1})")
    df_cnae_listas["lista_cnaes_2"] = df_cnae_listas["lista_cnaes_2"].str.extract(r"(\d+)").fillna("-1").astype("Int64")
    ...
    # Eztract section 
    df_cnae_limites["secao"] = df_cnae_limites["limite_inferior"].str.extract(r"(^[A-Z]{1})")
    # Extract limits numbers
    df_cnae_limites["limite_inferior"] = df_cnae_limites["limite_inferior"].str.extract(r"(\d+)").fillna("-1").astype("int")
    df_cnae_limites["limite_superior"] = df_cnae_limites["limite_superior"].str.extract(r"(\d+)").fillna("-1").astype("int")
    df_cnae_limites.loc[df_cnae_limites["limite_superior"]==-1,"limite_superior"] = None
    df_cnae_limites.loc[df_cnae_limites["limite_inferior"]==-1,"limite_inferior"] = None
    df_cnae_limites["limite_inferior"] = df_cnae_limites["limite_inferior"].astype("Int64")
    df_cnae_limites["limite_superior"] = df_cnae_limites["limite_superior"].astype("Int64")
    
    # For rows with null upper limit, fill the upper limit with the max division of the section
    df_cnae_limites.loc[df_cnae_limites["limite_superior"].isna(),"limite_superior"] = df_cnae_limites.loc[
        df_cnae_limites["limite_superior"].isna(),"secao"].apply(lambda x: df_diretorios.loc[df_diretorios["secao"]==x,"divisao"].max())
    for i, row in df_cnae_limites.iterrows():
        df_cnae_limites.at[i, "lista_cnaes_2"] = str(list(range(row["limite_inferior"], row["limite_superior"]+1)))
    # Explode cnaes list into rows and convert to string with leading zeros
    df_cnae_limites["lista_cnaes_2"] = df_cnae_limites["lista_cnaes_2"].apply(lambda x: eval(x))
    df_cnae_limites = df_cnae_limites.explode("lista_cnaes_2")
    df_cnae_limites["lista_cnaes_2"] = df_cnae_limites["lista_cnaes_2"].apply(lambda x: str(x).zfill(2))
    ...
    df_cnaes_expandidos["divisao_cnae"] = df_cnaes_expandidos.lista_cnaes_2.str.extract(r"(^\d{2})")
    df_cnaes_expandidos["grupo_cnae"] = df_cnaes_expandidos.lista_cnaes_2.str.extract(r"(^\d{3})")
    df_cnaes_expandidos["classe_cnae"] = df_cnaes_expandidos.lista_cnaes_2.str.extract(r"(^\d{5})")
    df_cnaes_expandidos["subclasse_cnae"] = df_cnaes_expandidos.lista_cnaes_2.str.extract(r"(^\d{7})")
    df_cnaes_expandidos = df_cnaes_expandidos.drop(columns=["limite_inferior", "limite_superior", "lista_cnaes_2"])
