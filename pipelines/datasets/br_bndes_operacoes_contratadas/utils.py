...
def download_xlsx(
        filename:str,
        input_path:Path|str,
        url:str=constants.URL_OPERACOES_CONTRATADAS.value)->Path:
    """
...
def get_xlsx_metadata(
    ...
    try:
        source_coverage_dates = [datetime.datetime.strptime(date, "%d/%m/%y") for date in re.findall(r"(\d{2,4}/\d{2,4}/\d{2,4})", metadata["metadados"][0])]
        last_source_update = datetime.datetime.strptime(re.findall(r"(\d{2,4}/\d{2,4}/\d{2,4})", metadata["metadados"][1])[0], "%d/%m/%y")
    except Exception as e:
        log(f"Error parsing dates with format %d/%m/%y: {e}. Trying with format %d/%m/%Y.")
        try:
            source_coverage_dates = [datetime.datetime.strptime(date, "%d/%m/%Y") for date in re.findall(r"(\d{2,4}/\d{2,4}/\d{2,4})", metadata["metadados"][0])]
            last_source_update = datetime.datetime.strptime(re.findall(r"(\d{2,4}/\d{2,4}/\d{2,4})", metadata["metadados"][1])[0], "%d/%m/%Y")
        except Exception as e:
            log(f"Error parsing dates with format %d/%m/%Y: {e}. Returning raw date strings.")
            source_coverage_dates = re.findall(r"(\d{2,4}/\d{2,4}/\d{2,4})", metadata["metadados"][0])
            last_source_update = re.findall(r"(\d{2,4}/\d{2,4}/\d{2,4})", metadata["metadados"][1])[0]
