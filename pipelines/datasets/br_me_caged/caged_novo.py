...
def creat_partition(df, save_clean_path, year_month_path):
    valid_ufs = [uf for uf in df["uf"].unique() if uf is not np.nan]
    for uf in valid_ufs:
        ano = year_month_path[0:4]
        mes = year_month_path[4:6]
        save_path = (
            save_clean_path
            + f"ano={int(ano)}/"
            + f"mes={int(mes)}/"
...
def rename_add_orginaze_columns(file_path, file_name, municipios):
    ...
    colunas_movimentacoes = {
        ...
        "origemdainformação": "fonte",
        "competênciamov": "competênciamov",
        "competênciadec": "competênciadec",
        "região": "região",
        "uf": "uf",
    }
