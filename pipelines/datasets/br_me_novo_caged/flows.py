...
from pipelines.datasets.br_me_novo_caged.tasks import (
    build_partitions,
    build_table_paths,
...
with Flow(
    "br_me_novo_caged.microdados_movimentacao", code_owners=["Luiza"]
) as br_me_novo_caged__microdados_movimentacao:
    dataset_id = Parameter(
        "dataset_id", default="br_me_novo_caged", required=True
    )
    ...
    # input_path, output_path = build_table_paths(table_id, upstream_tasks = [check_if_outdated])
    ...
    crawl_novo_caged_ftp.map(
        yearmonths,
        table_id,
        upstream_tasks=[unmapped(yearmonths)],
    )
...
br_me_novo_caged__microdados_movimentacao.storage = GCS(
    constants.GCS_FLOWS_BUCKET.value
)
br_me_novo_caged__microdados_movimentacao.run_config = KubernetesRun(
    image=constants.DOCKER_IMAGE.value
)
# cagedmov.schedule = every_month

# with Flow(
#     "br_me_novo_caged.check_caged_schedule", code_owners=["Luiza"]
# ) as br_me_novo_caged__get_schedule:
#     target = Parameter("target", default="prod", required=False)
#     table_ids = Parameter(
#         "table_ids",
#         default=[
...
#         required=False,
#     )
#     caged_schedule = get_caged_schedule()
#     check_if_outdated = check_if_data_is_outdated.map(
#         unmapped(dataset_id),
#         table_ids,
#         unmapped(caged_schedule[0]["competencia"]),
#         unmapped(date_format="%Y-%m"),
#         upstream_tasks=[unmapped(caged_schedule)],
#     )

#     with case(any(check_if_outdated), True):
#         table_ids = [table_ids[i] for i, check in enumerate(check_if_outdated) if check]
#         flow_names = [f"{dataset_id}.{table_id}" for table_id in table_ids]
...
# br_me_novo_caged__get_schedule.storage = GCS(constants.GCS_FLOWS_BUCKET.value)
# br_me_novo_caged__get_schedule.run_config = KubernetesRun(image=constants.DOCKER_IMAGE.value)
