...
with Flow(name="br_tse_eleicoes.apuracao", code_owners=["lucas_cr"]) as br_tse_apuracao:
    # Parameters
    dataset_id = Parameter("dataset_id", default="br_tse_eleicoes", required=True)
    table_id = Parameter("table_id", default="apuracao", required=True)
...
br_tse_apuracao.storage = GCS(constants.GCS_FLOWS_BUCKET.value)
br_tse_apuracao.run_config = KubernetesRun(image=constants.DOCKER_IMAGE.value)
br_tse_apuracao.schedule = schedule_apuracao
