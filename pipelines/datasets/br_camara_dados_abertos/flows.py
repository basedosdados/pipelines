...
with Flow(
    ...
    update_metadata = Parameter("update_metadata", default=True, required=False)
    data_source_max_date_proposicao = download_files_and_get_max_date_proposicao()

    dados_desatualizados = check_if_data_is_outdated(
        dataset_id=dataset_id,
        table_id=table_id[0],
        data_source_max_date=data_source_max_date_proposicao,
        date_format="%Y-%m-%d",
        upstream_tasks=[data_source_max_date_proposicao],
    )
