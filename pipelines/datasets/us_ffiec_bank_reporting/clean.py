...
from pipelines.datasets.us_ffiec_bank_reporting.dictionary import (
    rows as dictionary_rows,
)
from pipelines.datasets.us_ffiec_bank_reporting.mdrm import (
    is_flag_item,
    load_mdrm,
    parse_date,
)
from pipelines.datasets.us_ffiec_bank_reporting.schema_def import columns
...
def clean_call(
    ...
    """Melt Call Report schedules to the long fact table.

    `last` overrides the hardcoded CALL_LAST so the recurring flow can reach a
    quarter published after this module was written; `limit` then keeps only
    that many trailing quarters, which is how amendments get re-materialised
    without rebuilding seventeen years.
    """
    unmapped: Counter = Counter()
    unparsed: Counter = Counter()
    boolean: Counter = Counter()
    conflict_log: dict[str, int] = {}
    wanted = quarters(CALL_FIRST, last or CALL_LAST)
    if limit:
        wanted = wanted[-limit:]
...
def clean_bhc(
    ...
    """Melt FR Y-9C/Y-9SP extracts to the long fact table (see clean_call)."""
    unmapped: Counter = Counter()
    unparsed: Counter = Counter()
    boolean: Counter = Counter()
    wanted = quarters(BHC_FIRST, last or BHC_LAST)
    if limit:
        wanted = wanted[-limit:]
    for year, quarter in wanted:
...
def clean_cra(
    ...
    """Parse the CRA disclosure and transmittal files for a year range."""
