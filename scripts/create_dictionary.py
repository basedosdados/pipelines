...
def get_publisher_id(dataset_django_id:str, backend: bd.Backend):
    ...
    query_data = backend._execute_query(
        query=query,
        variables={"dataset_id":dataset_django_id}
        )
...
def create_dictionary_table_metadata(dataset_id:str, backend:bd.Backend) -> str:
    ...
    new_dict_parameters = {
        "slug":"dicionario",
        "name": "Dicionário",
        "description": f"Dicionário para tradução dos códigos das tabelas do do conjunto {dataset_django_info['name']}. Para códigos definidos por outras instituições, como id_municipio ou cnaes, buscar por diretórios",
    ...
    query_parameters = {
        "$dataset_Id: ID":dataset_django_id,
        "$slug: String":"dicionario"
    }

    _, id = create_update(mutation_class='CreateUpdateTable',
...
def create_dictionary_cloud_table_metadata(table_django_id:str, dataset_id:str, backend:bd.Backend) -> str:
    query_class = 'allCloudtable'
    query_parameters = {
        "$gcpDatasetId: String": dataset_id,
        "$gcpTableId: String": 'dicionario'
    }
    ...
    _, id = create_update(mutation_class, mutation_parameters,query_class, query_parameters, backend)
