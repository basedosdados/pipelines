...
def get_directory_column_id(directory_column_name, directory_table_name,backend) -> str:
    """
...
def select_columns(df,not_column_name, remove_already_done = False):
    ...
    df["column_identifier"] = df['table.dataset.fullSlug'].astype(str)+"-"+df['table.slug'].astype(str)+"."+df['name'].astype(str)
...
def link_directory_metadata(
    ...
    directory_column_id = get_directory_column_id(directory_column_name, directory_table_name,backend)
